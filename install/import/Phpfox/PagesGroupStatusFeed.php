 ...
 * @version    PagesGroupStatusFeed.php 2015-07-30 00:00:00Z john $
 * @author     John
 */
class Install_Import_Phpfox_PagesGroupStatusFeed extends Install_Import_Phpfox_Abstract
{
  ...
  {
    $this->_fromTable = $this->getFromPrefix() . 'pages_feed';
  ...
  protected function _translateRow(array $data, $key = null)
  {
    ...
    $typeId = $this->getFromDb()->select()
      ->from($this->getfromPrefix() . 'pages', 'type_id')
      ->where('page_id = ?', $data['parent_user_id'])
      ->query()
      ->fetchColumn(0);

    $type = 'post';
    $params = '';
    if( in_array($data['type_id'], array('blog', 'event', 'poll', 'video', 'music_album')) ) {
      return false;
    } else if( in_array($data['type_id'], array('forum', 'forum_post')) ) {
      if( $data['type_id'] == 'forum' ) {
        $type = 'group_topic_create';
        $params = '{"child_id":' . $data['item_id'] . '}';
      } else {

        $childId = $data['item_id'];
        $threadId = $this->getFromDb()->select()
          ->from('phpfox_forum_post', 'thread_id')
    ...
    $userId = $this->getFromDb()->query("SELECT  " . $this->getfromPrefix() . "pages.user_id FROM " . $this->getfromPrefix() . "user left join  " . $this->getfromPrefix() . "pages on page_id=profile_page_id where profile_page_id<>0 and  " . $this->getfromPrefix() . "user.user_id=" . $data['user_id'])->fetchColumn(0);
    if( $userId )
      $subject_id = $userId;
    ...
    if( $data['type_id'] == 'photo' ) {
      $body = $this->getFromDb()->select()
        ...
        ->query()
        ->fetchColumn();
    } else if( $data['type_id'] == 'link' ) {
      $body = $this->getFromDb()->select()
        ->from($this->getfromPrefix() . 'link', 'status_info')
        ->where('link_id = ?', $data['item_id'])
        ->query()
        ->fetchColumn();
    } else {
      $body = $this->getFromDb()->select()
        ->from($this->getfromPrefix() . 'pages_feed_comment', 'content')
    ...
    switch( $privacy ) {
      case 'friends':
        $targetTypes['owner'] = $subject_id;
        $targetTypes['parent'] = $subject_id;
        $targetTypes['members'] = $subject_id;
        break;
      case 'onlyme':
        $targetTypes['owner'] = $subject_id;
        $targetTypes['parent'] = $subject_id;
    ...
    $likes = $this->getFromDb()->select()
      ...
      ->fetchAll();
    $i = 0;
    $resourceType = $data['type_id'];
    if( $data['type_id'] == 'photo' )
      $resourceType = "group_photo";
    foreach( $likes as $likes ) {
      if( $data['type_id'] == 'link' || $data['type_id'] == 'pages_comment' ) {
    ...
    foreach( $comments as $comment ) {
      ...
      $body = $this->getFromDb()->select()
        ...
        ->fetchColumn();
      $parentCommentId = 0;
      if( $data['type_id'] == 'link' || $data['type_id'] == 'pages' ) {
