 ...
 * @version    $Id: prepare.tpl 9747 2012-07-26 02:08:08Z john $
 * @author     John
 */
?>
        ...
        <?php if ($this->transaction) : ?>
            <?php foreach ($this->transaction as $operation) : ?>
                <li>
                    ...
                    <?php endif; ?>
                    <?php echo $operation->getPrimaryPackage()->getVersion() ?>

                    <?php
                    $name = $currentPackage === null
                        ? $operation->getPrimaryPackage()->getName() : $currentPackage->getName();
                    ?>
                    <?php if (isset($this->warehouseErrors[$name])) { ?>
                    <div class="error">
                        <?php echo $this->warehouseErrors[$name]; ?>
                    </div>
                    <?php } ?>
                </li>
            <?php endforeach; ?>
            ...
            <ul>
                <?php foreach ($this->dependencies as $guid => $dependencies) : ?>
                    <?php foreach ($dependencies->getDependencies() as $dependency) :
                        if (!$this->settings['verbose'] && $dependency->getStatus() == 0) {
                            continue;
                        }
                        ?>
                        <li>
                            ...
                            } elseif ($dependency->getMinVersion()) {
                                echo '(at least ' . $dependency->getMinVersion() . ')';
                            } elseif ($dependency->getMaxVersion()) {
            ...
            <ul>
                <?php foreach ((array) $this->tests->getTests() as $packageTest): ?>
                    <?php foreach ($packageTest->getTests() as $test):
                        if (!$test->hasMessages() && !$this->settings['verbose']) {
                            continue;
                        }
                        ?>
                        <li>
            ...
            <div class="admin_packages_diff_container">
                <ul>
                    <?php foreach ($this->fileOperations as $batchSummary):
                        if (!$this->settings['verbose'] && empty($batchSummary['error'])) {
                            continue;
                        }
                        ?>
                        <?php foreach ($batchSummary['operations'] as $diffSummary):
                        if (!$this->settings['verbose'] && empty($diffSummary['error'])) {
                            continue;
                        }
                        ?>
                        <li>
