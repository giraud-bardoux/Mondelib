 ...
 * @version    $Id: InstallController.php 10024 2013-03-27 22:05:47Z john $
 * @author     John
 */
...
class InstallController extends Zend_Controller_Action
{
  ...
  public function init()
  {
    ...
    $this->view->layout()->inInstall = true;
...
//        'comparisonValue' => 1,
//        'messages' => array(
//          'badValue' => 'allow_url_fopen is currently OFF. This is OK, but it might prevent you from sharing files from other sites.',
//        ),
//      ),
      // PHP SAFE_MODE
      array(
    ...
    $sanityTestArray = _ENGINE_SSL ? array_merge($sanityTestArray, array( array(
      'type' => 'PhpExtension',
      'name' => 'OpenSSL',
      'extension' => 'openssl',
      'messages' => array(
        'noExtension' => 'The OpenSSL extension is required.',
      )
    ))) : $sanityTestArray;

    $this->view->test = $test = new Engine_Sanity(array(
      'basePath' => APPLICATION_PATH,
      'tests' => $sanityTestArray
    ));
    $test->run();
  ...
  public function dbInfoAction()
  {
    ...
    if ( Zend_Registry::isRegistered('Zend_Db') && $this->_getParam('clear', false) === false ) {
      $this->_session->mysql = true;
  ...
  public function dbCreateAction()
  {
    ...
    foreach( $packageManager->listInstalledPackages() as $package ) {
      if(!in_array($package->getName(), $preInstallModules)){
        continue;
      }
      if( $package->getType() === 'module' && in_array($package->getName(), $preInstallModules) ) {
        $adapter->update('engine4_core_modules', array(
    ...
    // Preprocess
      ...
      try {
        ...
        if( empty($_COOKIE[$mainSessionName]) ) {
          ...
          setcookie($mainSessionName, $mainSessionId, array ('path' => dirname($this->view->baseUrl()),'samesite' => 'Lax'));
        } else {
      ...
      try {
        $fieldValuesTable = new Zend_Db_Table(array(
          'db' => $adapter,
          'name' => 'engine4_user_fields_values',
        ));
  ...
  public function dbFormToConfig($formValues)
  {
    ...
    $params = $formValues;
    if ($adapter == 'mysqli') {
      $hostParts = explode(':', $params['host']);
      $params['host'] = $hostParts[0];
      $params['port'] = isset($hostParts[1]) ? $hostParts[1] : null;
    }
  ...
  protected function _createAuthFile(){
    $AuthSampleFilePath = APPLICATION_PATH . '/install/config/auth.sample.php';
    $AuthFilePath = APPLICATION_PATH . '/install/config/auth.php';
    $AuthSampleFileContent = file_get_contents($AuthSampleFilePath);
    if (file_exists($AuthSampleFilePath) && !file_exists($AuthFilePath)){
      @file_put_contents($AuthFilePath, $AuthSampleFileContent);
      @chmod($AuthFilePath,0777);
    }
  }
}
