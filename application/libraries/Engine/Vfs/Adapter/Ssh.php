...
class Engine_Vfs_Adapter_Ssh extends Engine_Vfs_Adapter_RemoteAbstract
{
  ...
  public function __sleep()
  {
    return array_merge(parent::__sleep(), array(
      '_privateKey', '_publicKey', '_hostKey', '_port'
    ));
  }
  ...
  public function put($path, $local)
  {
    ...
    array_pop($explode);
    $finalPath  = implode('/', $explode);
    $directory = $this->path($finalPath);
    if( !$this->isDirectory($directory) ) {
      $return = @ssh2_sftp_mkdir($this->getSftpResource(), $directory, $this->getUmask(0777), true);
