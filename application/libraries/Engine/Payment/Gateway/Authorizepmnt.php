 ...
 */
 include_once APPLICATION_PATH . "/application/libraries/Engine/Service/Authorize/autoload.php";
 use net\authorize\api\contract\v1 as AnetAPI; 
 use net\authorize\api\controller as AnetController;

class Engine_Payment_Gateway_Authorizepmnt extends Engine_Payment_Gateway
{
  // Support
  ...
  function test() {
    try {
      $customerProfileId="1929905607";
      $customerPaymentProfileId= "1842074814";
      $merchantAuthentication = new AnetAPI\MerchantAuthenticationType();
      $merchantAuthentication->setName($_POST["login_id"]);
      $merchantAuthentication->setTransactionKey($_POST["transaction_key"]);
      $refId = 'ref' . time();
      ...
      if($response->getMessages()->getMessage()[0]->getCode() != "E00040"){
        throw new Engine_Payment_Gateway_Exception(sprintf('Gateway login ' .
            ...
            '%1$s', $response->getMessages()->getMessage()[0]->getText()));
        }
    } catch (Exception $e) {
      throw new Engine_Payment_Gateway_Exception(sprintf('Gateway login ' .
            'failed. Please double-check ' .
