...
class Engine_Form_Element_CalendarDateTime extends Zend_Form_Element_Xhtml
{
  ...
  public function init()
  {
    ...
    $script = 'scriptJquery.datepicker.regional["'.$locale.'"] = {
      prevText: "'.Zend_Registry::get('Zend_Translate')->_('Prev').'",
      nextText: "'.Zend_Registry::get('Zend_Translate')->_('Next').'",
      monthNames: '.json_encode(array_values($months['format']['wide'])).',
      monthNamesShort: '.json_encode(array_values($months['format']['abbreviated'])).',
      dayNamesMin: '.json_encode(array_values($days['format']['narrow'])).',
    };
    scriptJquery.datepicker.setDefaults(scriptJquery.datepicker.regional["'.$locale.'"]);';
