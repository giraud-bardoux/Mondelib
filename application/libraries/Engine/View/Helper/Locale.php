...
class Engine_View_Helper_Locale extends Zend_View_Helper_Abstract
{
    ...
    /**
     * Get the locale according to viewer. Defaults to locale in registry
     *
     * @return Zend_Locale
     */
    ...
    /**
     * Get the default locale set by admin
     *
     * @return Zend_Locale
     */
    public function getLocaleDefault()
    {
        if (null === $this->_localeDefault) {
            $this->_localeDefault = Zend_Registry::get('LocaleDefault');
        }
     ...
     */
    public function _toNumber($number, $options = array())
    {
        if (!isset($options['default_locale'])) {
            $locale = $this->getLocale();
        } elseif ($options['default_locale']) {
            $locale = $this->getLocaleDefault();
    ...
    public function toNumber( $n, $options = array(), $precision = 1 ) {
        if ($n < 900) {
          // 0 - 900
          ...
          $suffix = $this->view->translate('CORE_NUMBER_FORMAT_K');
        } else if ($n < 900000000) {
          // 0.9m-850m
          $n_format = number_format($n / 1000000, $precision);
          $suffix = $this->view->translate('CORE_NUMBER_FORMAT_M');
        } else if ($n < 900000000000) {
          // 0.9b-850b
          $n_format = number_format($n / 1000000000, $precision);
          $suffix = $this->view->translate('CORE_NUMBER_FORMAT_B');
        } else {
          // 0.9t+
          $n_format = number_format($n / 1000000000000, $precision);
          $suffix = $this->view->translate('CORE_NUMBER_FORMAT_T');
        }
    ...
    protected function _checkDateTime($datetime, $options = array())
    {
        ...
        } elseif (!($datetime instanceof Zend_Date)) {
            return false;
        }
