...
class Engine_Filter_Html implements Zend_Filter_Interface
{
    ...
    public function execute($text)
    {
        ...
        $text = preg_replace_callback('/<(pre)(?:(?!<\/?\1).)*?<\/\1>/s', array($this, '_handlePre'), $text);
        ...
        if( $this->_pre ) {
            foreach( $this->_pre as $id => $data ) {
                $text = str_replace('[PRE="' . $id . '"]', $data, $text);
            }
        }
    ...
    private $_preIteration = 0;
    private $_pre = array();

    private function _handlePre($matches)
    {
        $text = $matches[0];

        /*Tinymce Code tag loses formatting when editing the post #1963 */
        $text = preg_replace('/<(\/?pre[^<>]*?)>/i', '[TO]$1[TC]', $text);
        $text = preg_replace('/<(\/?code[^<>]*?)>/i', '[TO]$1[TC]', $text);
        $text = str_replace(array('<', '>'), array('&lt;', '&gt;'), $text);
        $text = str_replace(array('[TO]', '[TC]'), array('<', '>'), $text);

       /* $text = preg_replace('/<pre(.*?)><code>/i', '', $text);
        $text = str_replace('</code></pre>', '', $text);
        $text = str_replace(array('<', '>'), array('&lt;', '&gt;'), $text);*/

        $this->_preIteration++;
        $this->_pre[$this->_preIteration] = $text;

        return '[PRE="' . $this->_preIteration . '"]';
    }
}
