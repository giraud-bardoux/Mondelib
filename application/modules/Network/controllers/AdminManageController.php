...
class Network_AdminManageController extends Core_Controller_Action_Admin
{
  ...
  public function settingsAction() {
    ...
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_manage_networks', array(), 'core_admin_main_networkssettings');

    $this->view->form = $form = new Network_Form_Admin_Global();
  ...
  {
    // Get navigation
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_manage_networks', array(), 'core_admin_main_managenetworks');
  ...
  public function editAction()
  {
    ...
    if( !$this->getRequest()->isPost() ) {
      $values = $network->toFormArray();
      if($values['pattern']['type'] == 'date') {
        $pattern = 'field_pattern_'.$values['field_id'];
        $values[$pattern] = (array) $values[$pattern];
      }
