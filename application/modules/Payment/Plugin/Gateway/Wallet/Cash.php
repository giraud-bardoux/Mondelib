...
class Payment_Plugin_Gateway_Wallet_Cash extends Engine_Payment_Plugin_Abstract
{
    ...
    if( $giveBenefit ) {
      ...
      if(engine_count($getAllAdmin) > 0) {
        ...
        foreach($getAllAdmin as $admin) {
          Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($admin, $user, $user, 'payment_manual_wallet', array('payment_method' => 'Cash','adminsidelink' => $adminSideLink));
  ...
  public function getGatewayUserForm(){
    ...
    $form->setDescription("Make the payment for the Wallet Subscription by Cash and then upload the receipt of your transaction.");
