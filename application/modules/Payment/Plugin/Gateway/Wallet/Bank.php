 ...
 * @license    http://www.socialengine.com/license/
 */
class Payment_Plugin_Gateway_Wallet_Bank extends Engine_Payment_Plugin_Abstract
{
  ...
  public function onWalletTransactionReturn(Payment_Model_Order $order, array $params = array()) {
    ...
    // description
    $desc = Engine_Api::_()->payment()->getPackageDescription($recurrence, $price, 'wallet');
    ...
    if( $giveBenefit ) {
      ...
      if(engine_count($getAllAdmin) > 0) {
        ...
        foreach($getAllAdmin as $admin) {
          Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($admin, $user, $user, 'payment_manual_wallet', array('payment_method' => 'Bank','adminsidelink' => $adminSideLink));
  ...
  public function getGatewayUserForm(){
    $form = new Payment_Form_Gateway_Bank(array('settings'=>$this->_gatewayInfo->config));
    $form->setDescription("Make the payment on the below given details of Bank and then upload the receipt of your transaction.");
  ...
  {
    return new Payment_Form_Admin_Gateway_Bank();
  }
