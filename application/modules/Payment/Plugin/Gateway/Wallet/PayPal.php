 ...
 * @license    http://www.socialengine.com/license/
 */
class Payment_Plugin_Gateway_Wallet_PayPal extends Engine_Payment_Plugin_Abstract
{
...
//     $price = @round(($price * $currencyChangeRate), 2);
  ...
  public function onWalletTransactionReturn(Payment_Model_Order $order, array $params = array()) {
    ...
    if( Engine_Api::_()->payment()->isOneTime($recurrence) ) {
      ...
      $params['type'] = "wallet recharge";
      $params['expiration_date'] = $expiration_date;
      $params['currency'] = Engine_Api::_()->payment()->defaultCurrency(); //this is default currency set by admin
      $params['amount'] = $price; //$subscriptionParams->price; // @todo use this or gross (-fee)?
      ...
      if( $paymentStatus == 'okay' ||
        ...
        if( $subscription->didStatusChange() ) {
          ...
          Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($user, $user, $user, 'payment_wallet_active', array('payment_method' => 'PayPal','walletlink' => $walletlink));
          ...
          if( Engine_Api::_()->payment()->isOneTime($recurrence) ) {
            switch( $rawData['payment_status'] ) {
              ...
              case 'Expired': // Not sure about this one
                ...
                // send notification
                if( $subscription->didStatusChange() ) {
                  Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'payment_wallet_expired', array(
                    'wallet_terms' => $desc,
                    'object_link' => 'https://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array("module" => 'payment', 'controller' => "settings", "action" => "wallet"), 'default', true),
                  ));
                }
          ...
          if( (!empty($rawData['initial_payment_status']) &&
            ...
            if( $subscription->didStatusChange() ) {
              Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'payment_subscription_active', array(
                ...
                'wallet_terms' => $package->getPackageDescription(),
