...
class Payment_Plugin_Gateway_Verification_Free extends Engine_Payment_Plugin_Abstract
{
   ...
   *
   * @return Engine_Payment_Gateway_Free
   */
  public function getGateway()
  {
    if( null === $this->_gateway ) {
      $class = 'Engine_Payment_Gateway_Free';
    ...
    if( $giveBenefit ) {
      ...
      if(engine_count($getAllAdmin) > 0) {
        ...
        foreach($getAllAdmin as $admin) {
          Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($admin, $user, $user, 'payment_manual_verification', array('payment_method' => 'Free','adminsidelink' => $adminSideLink));
  ...
  {
    return new Payment_Form_Admin_Gateway_Free();
  }
