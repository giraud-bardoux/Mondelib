 ...
 * @license    http://www.socialengine.com/license/
 */
class Payment_Plugin_Gateway_Verification_PayPal extends Engine_Payment_Plugin_Abstract
{
  ...
  public function createVerificationTransaction(User_Model_User $user, array $params = array()) {
    ...
    // Process description
    $desc = Engine_Api::_()->payment()->getPackageDescription(json_decode($recurrence), $price);
    if( strlen($desc) > 127 ) {
      ...
      $params['driverSpecificParams']['PayPal'] = array(
        ...
        'ITEMS' => array(
          array(
            ...
            'DESC' => $desc,
            'AMT' => $price, //$package->price,
            'NUMBER' => $params['vendor_order_id'], //$subscription->subscription_id,
            'QTY' => 1,
          ),
        )
  ...
  public function onVerificationTransactionReturn(Payment_Model_Order $order, array $params = array()) {
    ...
    // description
    $desc = Engine_Api::_()->payment()->getPackageDescription($recurrence, $subscriptionParams->price);
    if( strlen($desc) > 127 ) {
      $desc = substr($desc, 0, 124) . '...';
    } else if( !$desc || strlen($desc) <= 0 ) {
      $desc = 'N/A';
    ...
    if( Engine_Api::_()->payment()->isOneTime($recurrence) ) {
      ...
      $params['gateway_transaction_id'] = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];
      $params['state'] = $paymentStatus;
      $params['type'] = "payment verification";
      $params['expiration_date'] = $expiration_date;
      $params['currency'] = Engine_Api::_()->payment()->defaultCurrency(); //this is default currency set by admin
      $params['amount'] = $subscriptionParams->price; // @todo use this or gross (-fee)?
      $params['change_rate'] = $currencyChangeRate; //currency change rate according to default currency
      $params['current_currency'] = $current_currency; //$rdata['PAYMENTINFO'][0]['CURRENCYCODE']; //currency which is user paid
    ...
    // Recurring
      ...
      // Insert transaction
      $params['gateway_transaction_id'] = crc32(microtime() . $order->order_id);
      ...
      $params['amount'] = $subscriptionParams->price; // @todo use this or gross (-fee)?
      $params['change_rate'] = $currencyChangeRate; //currency change rate according to default currency
      $params['current_currency'] = Engine_Api::_()->payment()->getCurrentCurrency(); //currency which is user paid
      ...
      $rpData = array(
        ...
        'DESC' => $desc,
        'BILLINGPERIOD' => ucfirst($recurrence[1]), //ucfirst($package->recurrence_type),
        'BILLINGFREQUENCY' => $recurrence[0], //$package->recurrence,
        'INITAMT' => 0,
        'AMT' => $price, //$package->price,
        'CURRENCYCODE' => $this->getGateway()->getCurrency(),
      );
...
//       }
      ...
      $transaction->gateway_order_id = $profileId;
      //$transaction->current_currency = $rdata['PAYMENTINFO'][0]['CURRENCYCODE'];
      $transaction->save();
   ...
   * @param Engine_Payment_Ipn $ipn
   */
  public function onVerificationTransactionIpn(Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
    ...
    $params['expiration_date'] = $expiration_date;
    $params['currency'] = Engine_Api::_()->payment()->defaultCurrency(); //this is default currency set by admin
    $params['amount'] = $subscriptionParams['price']; //$rawData['mc_gross']; // @todo use this or gross (-fee)?
    $params['change_rate'] = $currencyChangeRate; //currency change rate according to default currency
    $params['current_currency'] = $rawData['currency_code']; //Engine_Api::_()->payment()->getCurrentCurrency(); //currency which is user paid
      ...
      switch( $rawData['txn_type'] ) {
          ...
          if( Engine_Api::_()->payment()->isOneTime($recurrence) ) {
            switch( $rawData['payment_status'] ) {
              ...
              case 'Reversed':
                ...
                // send notification
                if( $subscription->didStatusChange() ) {
                  Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'payment_verification_overdue', array(
                    'subscription_terms' => $desc,
                    'object_link' => 'https://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array("module" => 'payment', 'controller' => "settings", "action" => "verification"), 'default', true),
                  ));
                }
              ...
              case 'Refunded':
                ...
                // send notification
                if( $subscription->didStatusChange() ) {
                  Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'payment_verification_refunded', array(
                    'subscription_terms' => $desc,
                    'object_link' => 'https://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array("module" => 'payment', 'controller' => "settings", "action" => "verification"), 'default', true),
                  ));
                }
              ...
              case 'Expired': // Not sure about this one
                ...
                // send notification
                if( $subscription->didStatusChange() ) {
                  Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'payment_verification_expired', array(
                    'subscription_terms' => $desc,
                    'object_link' => 'https://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array("module" => 'payment', 'controller' => "settings", "action" => "verification"), 'default', true),
                  ));
                }
        ...
        case 'recurring_payment':
          if( !Engine_Api::_()->payment()->isOneTime($recurrence) ) {
            ...
            //update gateway_order_id in to transaction tablefor recurring payment
            $transaction->gateway_order_id = $order->gateway_order_id;
            $transaction->save();
            ...
            // send notification
            if( $subscription->didStatusChange() ) {
              Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'payment_verification_recurrence', array(
                'subscription_terms' => $desc,
                'object_link' => 'https://' . $_SERVER['HTTP_HOST'] . Zend_Controller_Front::getInstance()->getRouter()->assemble(array("module" => 'payment', 'controller' => "settings", "action" => "verification"), 'default', true),
              ));
            }
        ...
        case 'recurring_payment_expired':
          if( !Engine_Api::_()->qrcode()->isOneTime($recurrence) ) {
            $subscription->onExpiration();
            // send notification
            if( $subscription->didStatusChange() ) {
              Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'payment_verification_expired', array(
  ...
  public function cancelSubscription($transactionId, $note = null)
  {
    ...
    if( $transactionId instanceof Payment_Model_Subscription ) {
      $profileId = $transactionId->gateway_order_id;
    }
