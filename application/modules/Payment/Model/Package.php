 ...
 * @version    $Id: Package.php 9747 2012-07-26 02:08:08Z john $
...
class Payment_Model_Package extends Core_Model_Item_Abstract
{
    ...
    public function getAdditionalExpirationDate($rel = null)
    {
        if( null === $rel ) {
        $rel = time();
        }
        ...
        $relDate = new Zend_Date($rel);
        $relDate->add((int) $this->extra_day, Zend_Date::DAY);

        return $relDate->toValue();
    }

    public function getReminderDate($rel = null)
    {
        if( null === $rel ) {
        $rel = time();
        }
        ...
        $interval = $this->reminder_email;
        $interval_type = $this->reminder_email_type;
        }
        ...
        case 'year':
            ...
            break;
        default:
            throw new Engine_Payment_Exception('Invalid reminder_email_type');
            break;
        }

        $relDate = new Zend_Date($rel);
        $relDate->sub((int) $interval, $part);
  ...
  public function isPackageRecurrence() {

    if($this->recurrence_type == 'week' && $this->recurrence > 53) {
      return false;
    } else if($this->recurrence_type == 'year'  && $this->recurrence > 1) {
      return false;
    } else if($this->recurrence_type == 'month'  && $this->recurrence > 12) {
      return false;
    }
    return true;
  }
