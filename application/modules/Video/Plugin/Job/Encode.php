...
class Video_Plugin_Job_Encode extends Core_Plugin_Job_Abstract
{
  protected function _execute()
  {
    ...
    //Wall uplaod video
    if($video->uploadtype == 'wall' && empty($video->feedupload)) {
  ...
  protected function _process($video, $type, $compatibilityMode = false)
  {
    ...
    else
    {
      ...
      //Video on Demand
      if(Engine_Api::_()->getApi('settings', 'core')->getSetting('video_enable_videoondemad', 0)) {
        Engine_Api::_()->video()->createvideoondemand($video->file_id, $video->video_id);
      }
      ...
      //Multiple video work
      $action_id = 0;
      $action_id = Engine_Api::_()->getDbTable('attachments', 'activity')->getAttachmentId('video', $video->video_id);
      if(!empty($action_id)) {
        $video->status = 1;
        $video->save();
      ...
      try {
        if(empty($action_id)) {
          if($video->parent_type == 'group') {
            ...
            $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($owner, $group, 'video_new');
          } else {
            // new action
            if($video->parent_type != 'messages_message') {
        ...
        //Message video work
        if($video->parent_type == 'messages_message') {
          $getMessageId = Engine_Api::_()->getDbTable('messages', 'messages')->getMessageId(array('resource_type' => 'video', 'resource_id' => $video->getIdentity()));
          $video->parent_id = $getMessageId;
          $video->save();
        }
