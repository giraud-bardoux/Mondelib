 ...
 * @version    $Id: _activitycommentbodyoptions.tpl 2024-10-29 00:00:00Z 
 * @author     SocialEngine
 */
...
<ul class="comments_date"
            ...
            <?php foreach ($getReactions as $getReaction): ?>
              <span>
                <span data-text="<?php echo $this->translate($getReaction->title); ?>"
                  data-actionid="<?php echo $actionBody->getIdentity(); ?>"                   data-commentid="<?php echo $comment->getIdentity(); ?>" data-type="<?php echo $getReaction->reaction_id; ?>" data-guid="<?php echo $isPageSubject->getGuid(); ?>"
                  class="commentcommentlike reaction_btn comment_hoverbox_btn">
                  <div class="reaction comment_hoverbox_btn_icon"> <i class="react" style="background-image:url(<?php echo Engine_Api::_()->getDbTable('reactions', 'comment')->likeImage($getReaction->reaction_id); ?>)"></i>
                  </div>
                </span>
                <div class="text">
  ...
  <li class="sep">&middot;</li>
  <?php if ($this->viewer()->getIdentity() && $comment->poster_id != $this->viewer()->getIdentity() && $actionBody->type == "post_self_buysell"): ?>
    <li class="comments_reply">
      <?php echo $this->htmlLink($this->url(array('owner_id' => $comment->poster_id, 'action' => 'contact', 'controller' => 'index', 'module' => 'comment'), 'default', true), $this->translate('COREMESSAGE'), array('class' => 'ajaxsmoothbox')) ?>
    </li>
    <li class="sep">&middot;</li>
  <?php endif; ?>
    ...
    <?php if ($comment->preview && empty($comment->showpreview)) { ?>
      <li id="remove_preview_<?php echo $comment->comment_id ?>">
        <a href="javascript:void(0);"
          onclick="removePreview('<?php echo $comment->getIdentity(); ?>','<?php echo $comment->comment_id; ?>', '<?php echo $comment->getType(); ?>')">
