 ...
 * @version    $Id: _subjectcommentreply.tpl 2024-10-29 00:00:00Z 
 * @author     SocialEngine
 */
...
<?php if (empty($this->likeOptions)) { ?>
  <li id="comment-<?php echo $commentreply->comment_id; ?>">
        ...
        <?php if ($canDelete): ?>
          <div class="dropdown comment_pulldown_wrapper">
            ...
            <ul class="dropdown-menu dropdown-menu-end comment_pulldown_cont">
              ...
              <?php } ?>
              <?php //if (empty($commentreply->emoji_id) && empty($commentreply->gif_id)) { ?>
              ...
              <?php if ($this->viewer()->getIdentity() != $commentreply->poster_id) { ?>
                <li>
                  <?php echo $this->htmlLink(array("module" => "core", "controller" => "report", "action" => "create", "route" => "default", "subject" => $commentreply->getGuid()), '<span>' . $this->translate("Report") . '</span>', array('onclick' => "openSmoothBoxInUrl(this.href);return false;", "class" => "ajaxPrevent dropdown-item icon_report")); ?>
                </li>
              <?php } ?>
            </ul>
          </div>
      ...
      if (engine_count($likesGroup['data'])) {
        ...
        <span class="comments_likes_total">
          <span class="comments_likes_reactions">
            <?php foreach ($likesGroup['data'] as $type) { ?>
              <a title="<?php echo $this->translate('%s (%s)', $type['counts'], Engine_Api::_()->getDbTable('reactions', 'comment')->likeWord($type['type'])) ?>"
                ...
                data-url="<?php echo $this->url(array('module' => 'comment', 'controller' => 'ajax', 'action' => 'comment-likes', 'comment_id' => $commentreply->getIdentity(), 'id' => $action->getIdentity(), 'resource_type' => $action->getType(), 'format' => 'smoothbox'), 'default', true); ?>"><i
          ...
          <a href="javascript:;" class="ajaxsmoothbox"
            data-url="<?php echo $this->url(array('module' => 'comment', 'controller' => 'ajax', 'action' => 'comment-likes', 'comment_id' => $commentreply->getIdentity(), 'id' => $action->getIdentity(), 'resource_type' => $action->getType(), 'format' => 'smoothbox'), 'default', true); ?>"><?php echo $commentreply->likes()->getLikeCount(); ?></a>
        </span>
      <?php } ?>
    <?php endif ?>
    <ul class="comments_reply_date comments_date"
                ...
                <?php foreach ($getReactions as $getReaction): ?>
                  <span>
                    <span data-text="<?php echo $this->translate($getReaction->title); ?>"
                      data-sbjecttype="<?php echo $action->getType(); ?>" data-subjectid="<?php echo $action->getIdentity(); ?>"
      ...
      <li class="sep">&middot;</li>
      <?php if (strlen(preg_replace("/(\\\u[0-9a-f]{4})+?|\s+/", "", strip_tags($content)))) { ?>
        <!-- <li class="comments_reply_translate"> <a href="javascript:void(0);" class="comments_translate_link floatR"
            onClick="socialSharingPopUp('https://translate.google.com/#auto/<?php //echo $languageTranslate; ?>/<?php //echo urlencode(strip_tags($commentreply->body)); ?>','Google');return false;"><?php //echo $this->translate("Translate"); ?></a>
        </li>
        <li class="sep">&middot;</li> -->
      <?php } ?>
