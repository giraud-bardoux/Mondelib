 ...
 * @version    $Id: _subjectcommentreplycontent.tpl 2024-10-29 00:00:00Z 
 * @author     SocialEngine
 */
...
<?php if (empty($this->nolist)) { ?>
  <span class="comments_reply_body" id="comments_reply_body_<?php echo $commentreply->comment_id ?>"
    style="display:block;">
  <?php } ?>
  ...
  <span id="data-mention"
    style="display:none;"><?php echo json_encode($mentionUserData, JSON_HEX_QUOT | JSON_HEX_TAG); ?></span>
  <span class="comments_reply_body_actual" rel="" data-subject="<?php echo $commentreply->resource_type; ?>"
    data-subjectid="<?php echo $commentreply->resource_id; ?>" style="display:none;">
    <?php echo nl2br($commentreply->body); ?>
  </span>
  <?php
  if ($commentreply->file_id) { ?>
    <div class="comment_media_container clearfix">
      <?php
      $getFilesForComment = Engine_Api::_()->getDbTable('commentfiles', 'comment')->getFiles(array('comment_id' => $commentreply->comment_id));
      $counter = 0;
        ...
        if ($fileid->type == 'album_photo') {
          try {
            ...
            if ($photo) {
              ...
              ?>
              <div class="comment_reply_image comment_media_thumb clearfix" data-fileid="<?php echo $fileid->file_id; ?>"
                ...
                <a <?php if ($moduleEnable) { ?> class="core-image-viewer"
            ...
            if ($video) {
              ...
              <div class="comment_reply_image comment_media_thumb clearfix" data-fileid="<?php echo $fileid->file_id; ?>"
                ...
                <a class="sesvideo_thumb_img" href="<?php echo $video->getHref() ?>">
                  <span style="background-image:url(<?php echo $path; ?>);"></span><i class="comment_play_btn fa fa-play"></i></a>
              </div>
            <?php
            } else { ?>
              <div class="comment_image comment_media_thumb clearfix">
    ...
    <!-- <?php if ($counter > 2) { ?>
     <a href="javascript:void(0);" class="comment_media_more">See All</a>
    <?php } ?>-->
  <?php }
  ?>
  <?php if ($commentreply->gif_url && $commentreply->gif_url) { ?>
    <div class="emoji" data-rel="<?php echo $commentreply->gif_url; ?>">
      <img src="<?php echo $commentreply->gif_url; ?>" class="giphy_image" alt="<?php echo $commentreply->gif_url; ?>">
    </div>
  <?php } ?>
  <?php if ($commentreply->emoji_id) { ?>
    <div class="comment_image emoji" data-rel="<?php echo $commentreply->emoji_id; ?>">
      <?php
      $photo = Engine_Api::_()->getItem('comment_emotionfile', $commentreply->emoji_id);
      if ($photo) {
  ...
  <?php if ($commentreply->preview && empty($commentreply->showpreview)) { ?>
    <div class="comment_link_item" id="commentpreview_<?php echo $comment->comment_id; ?>">
      <?php
      $link = Engine_Api::_()->getItem('core_link', $commentreply->preview);
      if ($link) {
