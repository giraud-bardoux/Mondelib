...
class Forum_Model_Post extends Core_Model_Item_Abstract
{
  ...
  function getTitle() {
		$topic = Engine_Api::_()->getItem('forum_topic', $this->topic_id);
		return $topic->title;
	}
  ...
  public function getDescription()
  {
    ...
    if(!Engine_Api::_()->getApi('settings', 'core')->getSetting('decode_html') && Engine_Api::_()->getApi('settings', 'core')->getSetting('forum_bbcode')) {
      $content = $this->BBCode($content, array('link_no_preparse' => true));
      $content = Engine_Api::_()->core()->smileyToEmoticons($content);
    } else {
  ...
  {
    return Engine_Api::_()->getItem('forum', $this->forum_id);
  }
}
