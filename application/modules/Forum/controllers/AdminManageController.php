...
class Forum_AdminManageController extends Core_Controller_Action_Admin
{
  ...
  public function editForumAction()
  {
    ...
    if( $auth->isAllowed($forum, 'everyone', 'view') ) {
      ...
      if( engine_count($allowed) == 0 || engine_count($allowed) == engine_count($levels) ) {
        $allowed = null;
      }
    }
    if( !empty($allowed) ) {
    ...
    try {
      ...
      // Add
      if( engine_count($values['levels']) == 0 || engine_count($values['levels']) == engine_count($form->getElement('levels')->options) ) {
      ...
      $auth->setAllowed($forum, $list, 'topic_edit', true);
