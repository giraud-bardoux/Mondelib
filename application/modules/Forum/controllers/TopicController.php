...
class Forum_TopicController extends Core_Controller_Action_Standard
{
  ...
  public function deleteAction()
  {
    ...
    if(!Engine_Api::_()->authorization()->isAllowed('forum', $viewer->level_id, 'topic.delete'))
  ...
  public function viewAction()
  {
    ...
    if( !Engine_Api::_()->core()->isViewPermission($forum->getIdentity()) ) {
      return $this->_forward('notfound', 'error', 'core');
    }
    
    if( !$topic || !$topic->getIdentity() || ((!$topic->approved) && !$topic->isOwner($viewer)) ) {
    ...
    if($viewer->getIdentity() && $topic->user_id == $viewer->getIdentity()) {
    ...
    } else if($viewer->getIdentity() && $topic->user_id != $viewer->getIdentity() && $viewer->isSuperAdmin()) {
      $canEdit = true;
      $canDelete = true;
    } else if($viewer->getIdentity() && $topic->user_id != $viewer->getIdentity() && $viewer->isAdmin()) {
      $canEdit = Engine_Api::_()->authorization()->isAllowed('forum', $viewer->level_id, 'topic.edit');
      $canDelete = Engine_Api::_()->authorization()->isAllowed('forum', $viewer->level_id, 'topic.delete');
    }
    ...
    if(!$viewer->isAdmin())
      $select->where("CASE WHEN " .$tableName .".user_id = '".$viewer_id."' THEN true ELSE ".$tableName.".approved = 1 END ");
  ...
  public function stickyAction()
  {
    ...
    if(!Engine_Api::_()->authorization()->isAllowed('forum', $viewer->level_id, 'topic.edit'))
      return $this->_forward('requireauth', 'error', 'core');
  ...
  public function moveAction()
  {
    ...
    $multiOptions = array();
    $forums = Engine_Api::_()->getDbTable('forums', 'forum')->getForumsAssoc();
  ...
  public function postCreateAction()
  {
    ...
    try {
      ...
      Engine_Api::_()->core()->contentApprove($post, 'forum post');
      ...
      if ($post->approved) {
        ...
        foreach( $userTable->find($notifyUserIds) as $notifyUser ) {
          ...
          if( $notifyUser->isSelf($topicOwner) ) {
            ...
            if($topic->getUserPosts($notifyUser) > 0){
              $type = 'forum_topic_reply';
            } else {
              $type = 'forum_topic_subscribed';
            }
          }
