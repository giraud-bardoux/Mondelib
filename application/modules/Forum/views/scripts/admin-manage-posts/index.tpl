...
<p>
  <?php echo $this->translate("This page lists all of the topic post entries your users have posted. You can use this page to monitor these topics and delete offensive material if necessary.") ?><br>
  <?php
    ...
    <tbody>
      <?php foreach ($this->paginator as $item): ?>
        <tr>
          ...
          <td class="nowrap"  data-label="<?php echo $this->translate("Post") ?>"><?php echo $this->string()->truncate(strip_tags($item->body), 45, '...') ?></td>
          <td class="nowrap" data-label="<?php echo $this->translate("Topic Title") ?>"><a href="<?php echo $item->getParent()->getHref() ?>"><?php echo $item->getParent()->getTitle() ?></a></td>
          <td class="nowrap" data-label="<?php echo $this->translate("Forum Title") ?>"><a href="<?php echo $item->getParent()->getParent()->getHref() ?>"><?php echo $item->getParent()->getParent()->getTitle() ?></a></td>
          ...
          <td class="admin_table_options nowrap">
            <?php if(!$item->resubmit && empty($item->approved)) { ?>
              ...
              |
            <?php } ?>
            <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'forum', 'controller' => 'manage-posts', 'action' => 'readpost', 'id' => $item->getIdentity()), $this->translate("Read Post"),array('class' => 'ajaxsmoothbox')); ?>
              |
            <?php echo $this->htmlLink($item->getHref(), $this->translate('view')) ?>
              |
            <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'forum', 'controller' => 'manage-posts', 'action' => 'delete', 'id' => $item->getIdentity()), $this->translate("delete"),array('class' => 'smoothbox')) ?>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
