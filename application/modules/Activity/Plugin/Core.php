...
class Activity_Plugin_Core extends Zend_Controller_Plugin_Abstract {
  ...
  public function onActivityActionUpdateAfter($event) {
    ...
    if (engine_in_array('body', $payload->getModifiedFieldsName())) {
      $hashtags = Engine_Api::_()->activity()->getHashTags($payload->body);
    ...
    if (!engine_in_array('attachment_count', $payload->getModifiedFieldsName()) || $payload->attachment_count != 1) {
      return;
    }
  ...
  public function onItemDeleteBefore($event)
  {
    ...
    if( $item instanceof User_Model_User && $item->getType() == "user") {
      Engine_Api::_()->getDbtable('hides', 'activity')->delete(array(
        'resource_id = ?' => $item->getIdentity(),
      ));
      Engine_Api::_()->getDbtable('savefeeds', 'activity')->delete(array(
        'user_id = ?' => $item->getIdentity(),
      ));
    }
  ...
  public function addActivity($event)
  {
    ...
    // Network
    if( engine_in_array($content, array('everyone', 'networks')) ) {
      if( $object instanceof User_Model_User
    ...
    if (!engine_in_array($content, array('everyone', 'networks', 'friends', 'onlyme'))) {
  ...
  public function onActivityCommentCreateAfter($event)
  {
    ...
    $body = $payload['body'];
    $action = $activityApi->addActivity($viewer, $owner, 'comment_' . $commentedItem->getType(), $body, array(
      'owner' => $owner->getGuid(),
      'body' => $body
    ));
    if( $action ) {
  ...
  public function onActivityCommentDeleteBefore($event)
  {
    ...
    $activityApi = Engine_Api::_()->getDbtable('actions', 'activity');
    $activityApi->removeActivities($poster, $owner, 'comment_' . $commentedItem->getType());
  }
}
