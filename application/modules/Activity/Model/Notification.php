 ...
 * @version    $Id: Notification.php 2024-10-28 00:00:00Z 
 * @author     SocialEngine
 */
...
class Activity_Model_Notification extends Core_Model_Item_Abstract//extends Engine_Db_Table_Row
{
  ...
  {
    if( null === $this->_contentObject )
    {
      $typeInfo = $this->getTypeInfo();
      if(($this->object_id == $this->user_id) && $this->object_type == "user" && $typeInfo->module == "user"){
        $this->_contentObject = $this->getSubject();
      } else{
        $this->_contentObject = $this->getObject();
      } 
    }
