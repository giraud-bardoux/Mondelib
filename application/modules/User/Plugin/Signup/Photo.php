...
class User_Plugin_Signup_Photo extends Core_Plugin_FormSequence_Abstract
{
  ...
  public function onView()
  {
    ...
    if( !empty($_SESSION['facebook_signup']) && empty($_FILES["Filedata"]) ) {
      try {
    ...
    if (!empty($_SESSION['twitter_signup']) && empty($_FILES["Filedata"])) {
      try {
    ...
    if (!empty($_SESSION['google_signup']) && empty($_FILES["Filedata"])) {
      try {
    ...
    if (!empty($_SESSION['telegram_signup']) && empty($_FILES["Filedata"])) {
      try {
        $photo_url = $_SESSION['signup_fields']['photo'];
        $this->_fetchImage($photo_url);
      } catch (Exception $e) {
    ...
    if (!empty($_SESSION['linkedin_signup']) && empty($_FILES["Filedata"])) {
      try {
