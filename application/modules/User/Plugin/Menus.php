...
class User_Plugin_Menus {
  ...
  public function onMenuInitialize_CoreMinimenuSupportinbox($row)
  {
    $viewer = Engine_Api::_()->user()->getViewer();
    if( !$viewer->getIdentity()) 
      return false;
  ...
  public function onMenuInitialize_UserSettingsNetwork($row)
  {
    $viewer = Engine_Api::_()->user()->getViewer();
    if( !$viewer->getIdentity()) 
      return false;
  ...
  public function canDelete()
  {
    ...
    // Check auth
    return (bool) Engine_Api::_()->authorization()->getPermission($subject, 'user', 'delete');
  }
...
//       );
      ...
      if($controllerName == 'signup') {
      ...
      } else if($moduleName == 'core' && $controllerName == 'index' && $actionName == 'index') {
        return array(
          'class' => 'user_auth_link',
          'label' => 'Sign In',
          'route' => 'user_login',
        ...
        //$uri = !empty($_SESSION['return_url']) ? $_SESSION['return_url'] : '';
				$uri = $_SERVER['REQUEST_URI'];
        if($controllerName != 'auth' && $controllerName != 'signup' && strpos($uri,".js") === false && strpos($uri,".css") === false) {
           //'64-' . base64_encode($_SERVER['REQUEST_URI']);
          $_SESSION['return_url'] = $uri;
        }
        return array(
          'class' => 'user_auth_link',
          'label' => 'Sign In',
  ...
  public function onMenuInitialize_UserProfileBlock($row)
  {
    ...
    if( !Engine_Api::_()->authorization()->getPermission($viewer, 'user', 'block') ) {
      return false;
    }

    if( !Engine_Api::_()->authorization()->getPermission($subject, 'user', 'canblock') ) {
      return false;
    }
