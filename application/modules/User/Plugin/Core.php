...
class User_Plugin_Core extends Zend_Controller_Plugin_Abstract
{
  ...
  public function onItemCreateAfter($event)
  {
    ...
    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('core.followenable', '1')) {
      ...
      try {
        ...
        if ($type == 'activity_action' && $payload->type != 'status')
          return;
        $id = $payload->getIdentity();
        $viewer = Engine_Api::_()->user()->getViewer();
        $item = Engine_Api::_()->getItem($type, $id);
        if ($item && method_exists($item, 'follow')) {
          $followers = Engine_Api::_()->getDbTable('follows', 'user')->getFollowers($viewer->getIdentity());
          if (engine_count($followers) > 0) {
            $db = Engine_Db_Table::getDefaultAdapter();
            $db->query('INSERT IGNORE INTO `engine4_core_notificationrecipients` (`user_id`, `subject_type`, `subject_id`, `object_type`, `object_id`, `type`, `params`,`date`) SELECT f.resource_id , "user", f.user_id, "' . $item->getType() . '", "' . $item->getIdentity() . '", "user_follow_create", "", "' . date('Y-m-d H:i:s') . '" FROM engine4_user_follows f LEFT JOIN engine4_users u ON  u.user_id = f.user_id WHERE (f.user_id = "' . $viewer->getIdentity() . '");');
          }
        }
      } catch (Exception $e) {
      }
  ...
  public function onUserDeleteBefore($event)
  {
    ...
    if ($payload instanceof User_Model_User) {
      ...
      // Remove facebook/twitter/google/linkedin associations
      $db = Engine_Db_Table::getDefaultAdapter();
      $db->delete(
      ...
      $db->delete(
        'engine4_user_linkedin',
        array(
          'user_id = ?' => $payload->getIdentity(),
        )
      );
      $db->delete(
        'engine4_user_telegram',
        array(
          'user_id = ?' => $payload->getIdentity(),
        )
      );
...
//      }
  ...
  public function onActivityActionUpdateAfter($event)
  {
    ...
    if ($payload['type'] == 'status' && engine_in_array('body', $modifiedFields) && $oldData['body'] === $user->status) {
