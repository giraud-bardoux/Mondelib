...
class User_FriendsController extends Core_Controller_Action_User
{
  ...
  public function addAction()
  {
    ...
    if($viewer->member_count >= Engine_Api::_()->getApi('settings', 'core')->getSetting('user.friends.maxfriends', 5000)) {
      return $this->_forwardSuccess('You have already added the maximum number of friends.');
    }
  ...
  public function confirmAction()
  {
    ...
    if($format == 'json' && $viewer->member_count >= Engine_Api::_()->getApi('settings', 'core')->getSetting('user.friends.maxfriends', 5000)) {
      $this->view->status = false;
      $this->view->error = Zend_Registry::get('Zend_Translate')->_('You have already added the maximum number of friends.');
      return;
    } else if(!empty($format) && $viewer->member_count >= Engine_Api::_()->getApi('settings', 'core')->getSetting('user.friends.maxfriends', 5000)) {
      return $this->_forwardSuccess('You have already added the maximum number of friends.');
    }
    ...
    if( !$viewer->getIdentity() ) {
      ...
      foreach( $select->getTable()->fetchAll($select) as $friend ) {
        ...
        $data[] = array(
          ...
          'guid'  => $friend->getGuid(),
          'label' => $friend->getIdentity() == $viewer->getIdentity() ? $friend->getTitle(false) . ' (you)' : $friend->getTitle(false),
        ...
        $friend_data[$friend->getIdentity()] = $friend->getTitle(false);
      }
