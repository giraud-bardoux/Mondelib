...
class User_AdminSettingsController extends Core_Controller_Action_Admin
{
    ...
    public function friendsAction()
    {
      ...
      if( $this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost()) ) {
        ...
        if( $values['maxfriends'] > 5000 ) {
          return $form->addError('You have entered a value greater than 5000. Please enter a value less than 5000.');
        }
    ...
    public function followAction() {
      ...
      $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_settings_friends', array(), 'core_admin_main_follow');

      $this->view->form = $form = new User_Form_Admin_Settings_Follow();
      if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
        $values = $form->getValues();
    ...
    {
      $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_socialmenus', array(), 'core_admin_main_linkedin');
    
      $form = $this->view->form = new User_Form_Admin_Linkedin();
      $form->populate((array) Engine_Api::_()->getApi('settings', 'core')->core_linkedin);
      ...
      $values = $form->getValues();
      if( empty($values['access']) || empty($values['secret']) ) {
          $values['access'] = '';
          $values['secret'] = '';
      }

      Engine_Api::_()->getApi('settings', 'core')->core_linkedin = $values;
      $form->addNotice('Your changes have been saved.');
      $form->populate($values);
    }
    ...
    {
        $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_socialmenus', array(), 'core_admin_main_google');
      
        $form = $this->view->form = new User_Form_Admin_Google();
        $form->populate((array) Engine_Api::_()->getApi('settings', 'core')->core_google);
        ...
        Engine_Api::_()->getApi('settings', 'core')->core_google = $values;
        $form->addNotice('Your changes have been saved.');
        $form->populate($values);
    }
    public function telegramAction() {
        $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_socialmenus', array(), 'core_admin_main_telegram');
        $this->view->form = $form = new User_Form_Admin_Settings_Telegram();
        if( _ENGINE_ADMIN_NEUTER ) {
          $form->populate(array(
              'telegram_username' => '******',
              'telegram_token' => '******',
    ...
    {
        $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_socialmenus', array(), 'core_admin_main_twitter');
  ...
  public function socialshareAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_socialmenus', array(), 'core_admin_main_sharesettings');

    $form = $this->view->form = new User_Form_Admin_SocialShare();

    if ($this->getRequest()->isPost() && $form->isValid($this->_getAllParams())) {
      $values = $form->getValues();
      
      if($values['core_socialashare_allow'])
        $values['core_socialashare_allow'] = json_encode($values['core_socialashare_allow']);
