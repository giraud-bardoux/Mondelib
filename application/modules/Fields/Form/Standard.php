  ...
  public function generate()
  {
    ...
    foreach( $struct as $fskey => $map )
    {
      ...
      if($params['type'] == 'ProfileType') {
        unset($params['options']['multiOptions']['5']);
        unset($params['options']['multiOptions']['9']);
      }
      ...
      if( is_array($value) || $this->getElement($key)->isArray() ) {
        ...
        if( is_array($value) || !empty($value) ) {
          foreach( (array) $value as $singleValue ) {
            ...
            $valueRow->value = $censor->filter($singleValue);
...
//        }
        ...
        $valueRow->value = $censor->filter(htmlspecialchars($value));
        if( $this->_hasPrivacy && 
            !empty($this->_privacyValues[$key]) &&
            !empty($privacyOptions[$this->_privacyValues[$key]]) ) {
