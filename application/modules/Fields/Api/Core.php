...
class Fields_Api_Core extends Core_Api_Abstract
{
   ...
   * @return array
   */
  public function getFieldsValuesByAliasId(Core_Model_Item_Abstract $spec)
  {
    $values = array();
    ...
    foreach( $structure as $key => $map )
    {
      ...
      $alias = str_replace('_', '', $meta->alias);
      if($meta->alias == 'birthdate') {
        $alias = 'dob';
      }
      if($meta->alias == 'gender') {
        $gender = explode('_', $key);
        $gender = $gender[2];
        $getOptionValue = Engine_Api::_()->user()->getOptionValue(array('field_id' => $gender, 'label' => ucfirst($spec->gender)));
        $values[$key] = $getOptionValue;
      } else {
        if(isset($spec->$alias))
          $values[$key] = $spec->$alias;
      }
    }
    return $values;
  }
