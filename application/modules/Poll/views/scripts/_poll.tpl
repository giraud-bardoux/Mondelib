  ...
  en4.core.runonce.add(function() {
    var initializePoll = function() {
      ...
      en4.poll.addPollData(<?php echo $this->poll->getIdentity() ?>, {
        ...
        csrfToken : '<?php echo $this->voteHash($this->poll)->generateHash()?>',
        isClosed : <?php echo $this->poll->closed ? 'true' : 'false' ?>
      });

      scriptJquery('#poll_form_<?php echo $this->poll->getIdentity() ?> .poll_radio input').off('click').on('click', function(event) {
        en4.poll.vote(<?php echo $this->poll->getIdentity() ?>,this);
      });
    }
...
<span class="poll_view_single">
  <form id="poll_form_<?php echo $this->poll->getIdentity() ?>" action="<?php echo $this->url() ?>" method="POST" onsubmit="return false;">
    ...
    <div class="poll_stats">
      <a href='javascript:void(0);' onClick='en4.poll.toggleResults(<?php echo $this->poll->getIdentity() ?>); this.blur();' class="poll_toggleResultsLink font_color">
      ...
      <?php echo $this->htmlLink(array(
        ...
        'format' => 'smoothbox'
      ), $this->translate("Share"), array('class' => 'smoothbox font_color')); ?>
      &nbsp;|&nbsp;
      <?php echo $this->htmlLink(array(
        'module'=>'core',
        'controller'=>'report',
        ...
        'format' => 'smoothbox'
      ), $this->translate("Report"), array('class' => 'smoothbox font_color')); ?>
      <?php endif; ?>
      <?php if( $this->canEdit ): ?>
      &nbsp;|&nbsp;
      <?php echo $this->htmlLink(array(
      ...
      <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('poll.enable.location', 1) && Engine_Api::_()->getApi('settings', 'core')->getSetting('enableglocation', 0) && !empty($this->poll->location)) { ?>
        &nbsp;|&nbsp;
        <span><a href="<?php echo 'http://maps.google.com/?q='.$this->poll->location; ?>" target="_blank"><?php echo $this->poll->location; ?></a></span>
      <?php } ?>
    </div>
    <?php endif; ?>
  </form>
