...
<?php $levelId = $this->user->level_id;
  ...
  $coverphoto = Engine_Api::_()->authorization()->getPermission($levelId, 'poll', 'coverphoto');
...
<?php if (empty($this->uploadDefaultCover)) : ?>
  ...
  <?php elseif (!empty($coverId = Engine_Api::_()->getApi("settings", "core")->getSetting("pollcoverphoto.preview.level.id.$levelId"))) : ?>
    <div class="profile_cover_photo cover_photo_wap b_dark">
  ...
  <?php if (!empty($this->can_edit)) : ?>
    <div id="cover_photo_options" class="profile_cover_options">
      <ul class="edit-button">
        <li>
          <?php if (!empty($this->poll->coverphoto) && $this->photo) : ?>
          ...
          <ul class="profile_options_pulldown">
            <li>
              <a href='<?php echo $this->url(array('action' => 'upload-cover-photo', 'poll_id' => $pollId), 'poll_coverphoto', true); ?>'
            ...
            <li>
              <?php echo $this->htmlLink(
                array('route' => 'poll_coverphoto', 'action' => 'choose-from-albums', 'poll_id' => $pollId, 'recent' => 1),
            ...
            <?php if (!empty($this->poll->coverphoto) && $this->photo) : ?>
              ...
              <li>
                <?php echo $this->htmlLink(
                  array('route' => 'poll_coverphoto', 'action' => 'remove-cover-photo', 'poll_id' => $pollId),
      ...
      <?php if (!empty($this->poll->coverphoto)) : ?>
...
<?php else : ?>
  ...
  <div class="profile_cover_photo cover_photo_wap b_dark">
    <?php if(!empty($coverId = Engine_Api::_()->getApi("settings", "core")->getSetting("pollcoverphoto.preview.level.id.$levelId"))) : ?>
      <img src="<?php echo Engine_Api::_()->storage()->get($coverId, 'thumb.cover')->map()?>"
        align="left" class="cover_photo" style="top:<?php echo $this->topPosition?>px" />
        <div class="cover_tip_wrap is_hidden">
          <div class="cover_tip">
  ...
  <div id="cover_photo_options" class="profile_cover_options">
    <ul class="edit-button">
      <li>
        <?php if (Engine_Api::_()->getApi("settings", "core")->getSetting("pollcoverphoto.preview.level.id.$levelId")) : ?>
          <span class="profile_cover_btn">
            <i class="fa fa-camera" aria-hidden="true"></i>
          </span>
        <?php else : ?>
          ...
          <ul class="profile_options_pulldown">
            <li>
              <a href='<?php echo $this->url(array(
                ...
                'level_id' => $this->level_id), 'poll_coverphoto', true); ?>' class="profile_cover_icon_photo_upload smoothbox">
