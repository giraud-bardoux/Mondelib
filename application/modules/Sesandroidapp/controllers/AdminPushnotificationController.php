...
class Sesandroidapp_AdminPushnotificationController extends Core_Controller_Action_Admin {
 public function manageAction() {
   $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sesandroidapp_admin_main', array(), 'sesandroidapp_admin_main_pushnoti');
   
   $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sesandroidapp_admin_main_pushnoti', array(), 'sesandroidapp_admin_main_managepushnoti');
    
    $this->view->noti = Engine_Api::_()->getDbTable('pushnotifications','sesandroidapp')->getNotifications();
 }
 
 public function resendAction(){
   $id = $this->_getParam('id','');
   $item = Engine_Api::_()->getItem('sesandroidapp_pushnotifications',$id);
    $this->view->form = $form = new Sesandroidapp_Form_Admin_Notification();  
    $form->populate($item->toArray());  
      if ($this->getRequest()->isPost() && $form->isValid($_POST)) {
        ...
        $result = Engine_Api::_()->getApi('pushnoti','sesapi')->android($data,$tokens,array());
 ...
 public function createAction(){
    $this->view->form = $form = new Sesandroidapp_Form_Admin_Notification();    
      if ($this->getRequest()->isPost() && $form->isValid($_POST)) {
        ...
        return $this->_forward('success', 'utility', 'core', array(
          'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('route' => 'default','module' => 'sesandroidapp', 'controller' => 'pushnotification', 'action' => 'manage'),'admin_default',true),
  ...
  $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')
   ...
   $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sesandroidapp_admin_main_pushnoti', array(), 'sesandroidapp_admin_main_pushnotisettings');
    // Build the different notification types
    $modules = Engine_Api::_()->getDbtable('modules', 'core')->getModulesAssoc();
    $notificationTypes = Engine_Api::_()->getDbtable('notificationTypes', 'sesandroidapp')->getNotificationTypes();
    $notificationSettings = Engine_Api::_()->getDbtable('notificationTypes', 'sesandroidapp')->getDefaultNotifications();
    ...
    Engine_Api::_()->getDbtable('notificationTypes', 'sesandroidapp')
