            ...
            ?>
              <li class="item_label" id="slide_<?php echo $item->getIdentity(); ?>">
                ...
                <div style="width:10%;" class="admin_table_centered">
                  ...
                  <?php } ?>
                  <?php if($enable){ ?>
                  <?php echo ( $item->status ? $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesandroidapp', 'controller' => 'menu', 'action' => 'status', 'id' => $item->getIdentity()), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sesandroidapp/externals/images/admin/check.png', '', array('title' => $this->translate('Disable'))), array()) : $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesandroidapp', 'controller' => 'menu', 'action' => 'status', 'id' => $item->getIdentity()), $this->htmlImage('application/modules/Sesandroidapp/externals/images/admin/error.png', '', array('title' => $this->translate('Enable')))) ) ?>
                  <?php }else{ ?>
                ...
                <div style="width:15%;">          
                  <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesandroidapp', 'controller' => 'menu', 'action' => 'create', 'id' => $item->getIdentity()), $this->translate("Edit"), array('class'=>'smoothbox')) ?> |
                  <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesandroidapp', 'controller' => 'menu', 'action' => 'info', 'id' => $item->getIdentity()), $this->translate("Info"), array('class'=>'smoothbox')) ?>
                  <?php if($item->is_delete == 1 && !$item->class){ ?>
              |
            <?php echo $this->htmlLink(
                array('route' => 'admin_default', 'module' => 'sesandroidapp', 'controller' => 'menu', 'action' => 'delete', 'id' => $item->getIdentity()),
