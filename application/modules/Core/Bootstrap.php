...
class Core_Bootstrap extends Engine_Application_Bootstrap_Abstract
{
    ...
    protected function _initLog()
    {
        ...
        switch ($logAdapter) {
            ...
            case 'file': {
                try {
                    if(Engine_Api::_()->getApi('settings', 'core')->getSetting('core.empty.log',0) || !_ENGINE_ENABLE_LOG) {
                        //Clear Error Logs
        ...
        */
        ...
        // Use CSV Translation Adapter
            ...
            if(!array_key_exists($language, $languageList)) {
              $language = Engine_Api::_()->getApi('settings', 'core')->getSetting('core.locale.locale', 'en');
              if($language == 'auto')
              $language = 'en';
