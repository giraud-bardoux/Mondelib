 ...
 * @version    $Id: AdminTasksController.php 9747 2012-07-26 02:08:08Z john $
...
class Core_AdminTasksController extends Core_Controller_Action_Admin
{
  ...
  public function jobsAction()
  {
    ...
    if( !empty($filterValues['state']) ) {
      $select->where('state = ?', $filterValues['state']);
    }
    if( !empty($filterValues['order']) ) {
      if( empty($filterValues['direction']) ) {
        $filterValues['direction'] = 'DESC';
  ...
  public function jobRetryAction()
  {
    ...
    // Retry job
    if( engine_in_array($job->state, array('failed', 'cancelled', 'timeout')) ) {
  ...
  public function jobCancelAction()
  {
    ...
    // Cancel job
    if( engine_in_array($job->state, array('active', 'sleeping', 'pending')) ) {
