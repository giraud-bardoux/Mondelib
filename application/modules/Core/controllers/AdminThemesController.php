...
class Core_AdminThemesController extends Core_Controller_Action_Admin
{
    public function indexAction()
    {
        ...
        if (null !== ($rFile = $this->_getParam('file'))) {
            if (engine_in_array($rFile, $manifests[$activeTheme->name]['files'])) {
    ...
    public function saveAction()
    {
        ...
        $body = str_replace('content:"\f', 'content:"\\f',$body);
        $body = str_replace("content:'\f", "content:'\\f",$body);
        ...
        if (isset($manifestData['less'])
            && is_array($manifestData['less'])
            && engine_in_array($file, $manifestData['less'])
            && defined('_ENGINE_HAS_VENDOR')
        ) {
            $less = file_get_contents($basePath . '/main.less');
            $less .= $body;
            $parser = new Less_Parser;
            $parser->SetOption('compress', true);
            $parser->parse($less, $basePath . '/');
            $css = $parser->getCss();

            file_put_contents($basePath . '/main.css', $css);
        }
    ...
    public function cloneAction()
    {
        ...
        if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
            ...
            try {
                ...
                Engine_Api::_()->getDbtable('themes', 'core')->insert(array(
                    'name'  => $new_theme['name'],
                    'title' => $new_theme['title'],
                    'description' => $new_theme['description'],
                ));
    ...
    public function deleteAction()
    {
        ...
        try {
            ...
            Engine_Api::_()->getDbtable('themes', 'core')->delete(array(
                'name = ?' => $this->view->name,
            ));
            ...
            $this->_forward('success', 'utility', 'core', array(
                ...
                'parentRefresh' => true,
                'format' => 'smoothbox',
                'messages' => array(Zend_Registry::get('Zend_Translate')->_("Theme Deleted"))
            ));
        } catch (Exception $e) {
            $this->_forward('success', 'utility', 'core', array(
                'smoothboxClose' => true,
    ...
    public function createColorVariantAction()
    {
        $selectedTheme = $this->_getParam('name');
        $themes = Engine_Api::_()->getDbtable('themes', 'core')->fetchAll();
        if (!($themes->getRowMatching('name', $selectedTheme))) {
            throw new Engine_Exception("Theme not found: " . $selectedTheme);
        }

        $masterDirectory = APPLICATION_PATH . '/application/themes/' . $selectedTheme;
        $manifest = include("$masterDirectory/manifest.php");
        $colorVariants = $manifest['colorVariants'];

        foreach ($colorVariants as $key => $val) {
            $colorVariants[$key]['name']        = $key;
            $colorVariants[$key]['thumb']       = $manifest['package']['thumb'];
            $colorVariants[$key]['version']     = $manifest['package']['version'];
            $colorVariants[$key]['path']        = $manifest['package']['path'] . "/color-variants/" . $key;
            $colorVariants[$key]['parentTheme'] = $manifest['package']['name'];
        }

        $activeTheme = $themes->getRowMatching('active', 1);

        $form = $this->view->form = new Core_Form_Admin_Themes_CreateColorVariant(array(
            'colorVariants' => $colorVariants,
            'activeTheme' => $activeTheme,
        ));
        $form->getElement('themeName')->setValue($selectedTheme);
        ...
        $selectedColorVariant = $formData['colorVariantName'];
        if (!array_key_exists($selectedColorVariant, $colorVariants)) {
            throw new Engine_Exception($formData['colorVariantName'] . " color variant not found in " . $selectedTheme . "theme");
        }

        $newTheme = $colorVariants[$selectedColorVariant];

        $newDirectory = dirname($masterDirectory) . '/' . $newTheme['name'];

        // Copy whole directory of parent theme
        Engine_Package_Utilities::fsCopyRecursive($masterDirectory, $newDirectory);
        chmod($newDirectory, 0777);
        foreach (self::rscandir($newDirectory) as $file) {
            chmod($file, 0777);
        }

        // Replace files from color-variants directory files
        $colorVariantDirectory = APPLICATION_PATH . "/" . $newTheme['path'];
        Engine_Package_Utilities::fsCopyRecursive($colorVariantDirectory, $newDirectory);
        chmod($newDirectory, 0777);
        foreach (self::rscandir($newDirectory) as $file) {
            chmod($file, 0777);
        }

        // Delete color-variants directory from newly created theme
        Engine_Package_Utilities::fsRmdirRecursive($newDirectory . "/color-variants", true);

        $newManifest = include("$newDirectory/manifest.php");

        if (isset($newManifest['package']['meta'])) {
            $newManifest['package'] = array_merge($newManifest['package']['meta'], $newManifest['package']);
            unset($newManifest['package']['meta']);
        }

        $newManifest['package']['name'] = $newTheme['name'];
        $newManifest['package']['title'] = $newTheme['title'];
        $newManifest['package']['version'] = $newTheme['version'];
        $newManifest['package']['path'] = substr($newDirectory, 1 + strlen(APPLICATION_PATH));
        $newManifest['package']['directories'][0] = substr($newDirectory, 1 + strlen(APPLICATION_PATH));
        $newManifest['colorVariants'] = array();

        file_put_contents("$newDirectory/manifest.php", '<?php return ' . var_export($newManifest, true) . '; ?>');
        ...
        try {
            ...
            if (isset($formData['activate']) && $formData['activate'] == 'activate') {
                $themeTable->update(array(
                    'active' => 0,
                ), array(
                    'active = ?' => 1,
            ...
            $themeSelect = $themeTable->select()
                ...
                ->limit(1)
            ;
            $res = $themeTable->fetchRow($themeSelect);

            if (!$res) {
                $row = $themeTable->createRow();
                $row->name = $newTheme['name'];
                $row->title = $newTheme['title'];
                $row->active = $active;
                if (isset($newTheme['description'])) {
                    $row->description = $newTheme['description'];
                }
        ...
        if (isset($formData['formatType']) && $formData['formatType'] == 'smoothbox') {
            return $this->_forward('success', 'utility', 'core', array(
                'messages' => array(Zend_Registry::get('Zend_Translate')->_('Theme is being upgraded...')),
                'layout' => 'admin-simple',
                'parentRefresh' => 3000,
                'smoothboxClose' => 3000
            ));
    ...
    public function upgradeColorVariantAction()
    {
        $parentThemeName = $this->_getParam('name');
        $colorVariantName = $this->_getParam('colorVariantName');

        $form = $this->view->form = new Core_Form_Admin_Themes_UpgradeColorVariant(array('colorVariantName' => $colorVariantName));
        $form->setAction($this->view->url(array('action' => 'create-color-variant', 'name' => $parentThemeName)));
    }
}
