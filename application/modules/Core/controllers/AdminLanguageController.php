 ...
 * @version    $Id: AdminLanguageController.php 9760 2012-08-13 23:35:28Z matthew $
 * @author     Jung
 */
...
class Core_AdminLanguageController extends Core_Controller_Action_Admin
{
    ...
    public function editAction()
    {
        ...
        } elseif ($localeLanguageTranslation) {
            $this->view->localeTranslation = $localeLanguageTranslation . ' '
                . sprintf($translate->translate('[%s]'), $locale);
        } else {
        ...
        foreach ($baseMessages as $key => $value) {
            ...
            if (isset($currentMessages[$key])) {
                ...
                } elseif (!is_array($value) && is_array($currentMessages[$key])) {
    ...
    public function translateRemoteAction()
    {
        ...
        if (engine_in_array($target, array('all', 'all-translated', 'all-untranslated'))) {
            ...
            } elseif ($target == 'all-untranslated') {
        ...
        if (empty($file)) {
            ...
            return;
        } elseif (!engine_in_array($file, $files)) {
            $this->view->status = false;
        ...
        if ($retranslate) {
            $this->view->newOffset = $offset = ($offset + engine_count($currentMessages));
        }
        ...
        $this->view->status = true;
        $this->view->sourceCount = engine_count($sourceMessages);
        $this->view->targetCount = engine_count($targetMessages);
        $this->view->untranslatedCount = engine_count($deltaMessages);
        $this->view->translatedCount = engine_count($resultantMessages);
    }
    ...
    protected function _restoreAll($char, $data, $string)
    {
        // <![CDATA[
        // ]]>
        if (is_array($data) && engine_count($data) > 0) {
