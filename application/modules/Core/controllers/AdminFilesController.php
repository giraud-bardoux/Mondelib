...
class Core_AdminFilesController extends Core_Controller_Action_Admin
{
  ...
  public function settingsAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_layout_files', array(), 'core_admin_main_layout_files_settings');

    $this->view->form = $form = new Core_Form_Admin_File_Settings();
  ...
  public function indexAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('core_admin_main_layout_files', array(), 'core_admin_main_layout_files_manager');
    
    $this->view->path = $path = $this->_getPath();
    $this->view->existingFiles = engine_count(array_diff(scandir($path), array('..', '.', 'index.html')));
  ...
  public function downloadAction() {
    ...
    if($storage->service_id != 1) {
      ...
      $name = urlencode($file->name.'.'.$file->extension);
  ...
  public function uploadAction()
  {
    ...
    $name = end($parts);
    if( is_array($name) && engine_in_array($name, $disallowedExtensions) ) {
    ...
    try {
      ...
      if(!empty($info['name'])) {
        ...
        $row->storage_path = $storageObject->storage_path;
        $row->storage_file_id = $storageObject->file_id;
        $row->extension = $storageObject->extension;
        $row->save();
      }
      $db->commit();
    } catch(Exception $e) {
