...
<?php echo $this->partial('_admin_breadcrumb.tpl', 'core', array('parentMenu' => "core_admin_main_layout", 'childMenuItemName' => 'core_admin_main_layout_content')); ?>
...
<script type="text/javascript">
  ...
  var pushModification = function(type) {
    if( !currentModifications.contains(type) ) {
      ...
      if( type == 'info' ) {
        layoutAdmin('admin_layoutbox_menu_pageinfo').addClass('admin_content_modifications_active');
      } else if( type == 'main' ) {
        layoutAdmin('admin_layoutbox_menu_savechanges').addClass('admin_content_modifications_active');
      }
    }
  }
  ...
  var eraseModification = function(type) {
    ...
    // Remove active notifications CSS class
      if( type == 'info' ) {
        layoutAdmin('admin_layoutbox_menu_pageinfo').removeClass('admin_content_modifications_active');
      } else if( type == 'main' ) {
        layoutAdmin('admin_layoutbox_menu_savechanges').removeClass('admin_content_modifications_active');
      }
  }
  ...
  var saveChanges = function()
  {
    ...
    $$('.admin_content_buildable').each(function(element) {
      ...
      //changes for double quote in string.
      var contentInfo = element.retrieve('contentInfo');
      if(contentInfo) {
        contentInfo.title = '';
        contentInfo.description = '';
        contentInfo.lable = '';
        
        if(contentInfo.adminForm) {
          var adminForm = contentInfo.adminForm;
          if(adminForm.elements) {
            var adminFormelements  = adminForm.elements;
            if(adminFormelements.length > 0) {
              var newAdminForm = [];
              adminFormelements.forEach(item => {
                if(item){
                  if(item[2]) {
                    if(item[2].label != 'undefined')
                    item[2].label = '';
                    if(item[2].description != 'undefined')
                    item[2].description = '';
                    if(item[2].multiOptions) {
                      for (var key in item[2].multiOptions) {
                        item[2].multiOptions[key] = '';
                      }
                    }
                  }
                }
                newAdminForm.push(item);
              }) ;
              contentInfo.adminForm = newAdminForm;
            }
          }
        }
      }
      elData.info = contentInfo;
    ...
    var request = new Request.JSON({
      ...
      onComplete : function(responseJSON) {
        $H(responseJSON.newIds).each(function(data, index) {
          var newContentEl = layoutAdmin('admin_content_new_' + index);
  ...
  var openWidgetParamEdit = function(name, element) {
    ...
    currentEditingElement = layoutAdmin(element);
    var content_id;
...
<h2 class="page_heading"><?php echo $this->translate('Layout Editor'); ?></h2>
<p>
  <?php echo $this->translate('CORE_LAYOUT_DESC'); ?>
  <?php
  $settings = Engine_Api::_()->getApi('settings', 'core');
  if( $settings->getSetting('user.support.links', 0) == 1 ) {
          echo 'More info: <a href="https://community.socialengine.com/blogs/597/61/layout-editor" target="_blank">See KB article</a>';
  } 
  ?>	
</p>
<br />	
...
<div id='admin_cms_wrapper'>
  ...
  <div class="admin_layoutbox_menu">
    <ul>
      <li class="admin_layoutbox_menu_generic" id="admin_layoutbox_menu_openpage">
        ...
        <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), layoutAdmin(this));">
 ...
 <div class="admin_layoutbox_items">
          ...
          <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_editinfo_wrapper" id="admin_layoutbox_menu_editinfo_wrapper">
            <div class="admin_layoutbox_menu_editinfo">
              ...
              <div class="admin_layoutbox_menu_editinfo_submit">
                <button onclick="saveCurrentPageInfo(layoutAdmin('admin_content_pageinfo')); return false;"><?php echo $this->translate("Save Changes") ?></button> or <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), layoutAdmin(this));"><?php echo $this->translate("cancel") ?></a>
              </div>
            </div>
          </div>
          <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), layoutAdmin(this));"><?php echo $this->translate("Edit Page Info") ?></a>
        </li>
          ...
          <div class="admin_layoutbox_menu_wrapper_generic admin_layoutbox_menu_columnchoices_wrapper" id="admin_layoutbox_menu_columnchoices_wrapper">
            <div class="admin_layoutbox_menu_columnchoices">
              ...
              <ul class="admin_layoutbox_menu_columnchoices_thumbs">
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols1_3.png', '3 columns', array('onClick' => "changeCurrentLayoutType('left,middle,right');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols1_2left.png', '2 columns - Left', array('onClick' => "changeCurrentLayoutType('left,middle');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols1_2right.png', '2 columns - Right', array('onClick' => "changeCurrentLayoutType('middle,right');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols1_1.png', '1 columns', array('onClick' => "changeCurrentLayoutType('middle');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
              </ul>
              <ul class="admin_layoutbox_menu_columnchoices_thumbs">
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols2_3.png', '3 columns', array('onClick' => "changeCurrentLayoutType('top,left,middle,right');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols2_2left.png', '2 columns - Left', array('onClick' => "changeCurrentLayoutType('top,left,middle');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols2_2right.png', '2 columns - Right', array('onClick' => "changeCurrentLayoutType('top,middle,right');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols2_1.png', '1 columns', array('onClick' => "changeCurrentLayoutType('top,middle');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
              </ul>
              <ul class="admin_layoutbox_menu_columnchoices_thumbs">
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols3_3.png', '3 columns', array('onClick' => "changeCurrentLayoutType('left,middle,right,bottom');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols3_2left.png', '2 columns - Left', array('onClick' => "changeCurrentLayoutType('left,middle,bottom');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols3_2right.png', '2 columns - Right', array('onClick' => "changeCurrentLayoutType('middle,right,bottom');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols3_1.png', '1 columns', array('onClick' => "changeCurrentLayoutType('middle,bottom');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
              </ul>
              <ul class="admin_layoutbox_menu_columnchoices_thumbs">
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols4_3.png', '3 columns', array('onClick' => "changeCurrentLayoutType('top,left,middle,right,bottom');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols4_2left.png', '2 columns - Left', array('onClick' => "changeCurrentLayoutType('top,left,middle,bottom');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols4_2right.png', '2 columns - Right', array('onClick' => "changeCurrentLayoutType('top,middle,right,bottom');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
                <li><?php echo $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Core/externals/images/content/cols4_1.png', '1 columns', array('onClick' => "changeCurrentLayoutType('top,middle,bottom');switchPageMenu(new Event(event), layoutAdmin(this));")) ?></li>
              </ul>
              <div class="admin_layoutbox_menu_columnchoices_cancel">
                Or, <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), layoutAdmin(this));"><?php echo $this->translate("cancel") ?></a> <?php echo $this->translate("and keep your current layout.") ?>
              </div>
            </div>
          </div>
          <a href="javascript:void(0);" onClick="switchPageMenu(new Event(event), layoutAdmin(this));"><?php echo $this->translate("Edit Columns") ?></a>
        </li>
        <?php endif ;?>
      </ul>
    </div>
      ...
      <div class='admin_layoutbox'>
        <div class='admin_layoutbox_header<?php echo ( empty($this->pageObject->layout) || $this->pageObject->layout == 'default' ? '' : ' admin_layoutbox_header_hidden' ) ?>'>
          <span>
            ...
            <span>
              <a href="javascript:void(0);" onclick="toggleGlobalLayout(layoutAdmin(this).getParent('div.admin_layoutbox_header'));">
                ...
                <tr>
                  <script type="text/javascript">
                    window.addEvent('domready', function() {
                      layoutAdmin("admin_content_<?php echo $structOne['identity'] ?>").store('contentParams', <?php echo Zend_Json::encode($structOneNE) ?>);
                    });
                  </script>
                  <?php
                  ...
                  ?>
                    <td id="admin_content_<?php echo $structTwo['identity'] ?>" class="admin_content_column admin_content_buildable admin_content_<?php echo $structTwo['type'] . '_' . $structTwo['name'] ?>">
                      <script type="text/javascript">
                        window.addEvent('domready', function() {
                          layoutAdmin("admin_content_<?php echo $structTwo['identity'] ?>").store('contentParams', <?php echo Zend_Json::encode($structTwoNE) ?>);
                        });
                      </script>
                      <ul class="admin_content_sortable">
                        ...
                        ?>
                          <script type="text/javascript">
                            window.addEvent('domready', function() {
                              layoutAdmin("admin_content_<?php echo $structThree['identity'] ?>").store('contentParams', <?php echo Zend_Json::encode($structThreeNE) ?>);
                            });
                          </script>
                          <?php if( empty($structThreeInfo) ): // Missing widget ?>
                            <li style="display:none;" id="admin_content_<?php echo $structThree['identity'] ?>" class="disabled admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?><?php if( !empty($structThreeInfo['special']) ) echo ' htmlblock' ?>">
                          ...
                          <?php elseif( empty($structThreeInfo['canHaveChildren']) ): ?>
                            <li id="admin_content_<?php echo $structThree['identity'] ?>" class="admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?><?php if( !empty($structThreeInfo['special']) ) echo ' htmlblock' ?>">
                              <?php
                                // showing admin title
                                if( engine_in_array($structThree['name'], $allowedAdminTitle) && (!empty($structThreeNE['params']['adminTitle'])) ):
                              ...
                              <span class="open">
                                |
                                <a href='javascript:void(0);' onclick="openWidgetParamEdit('<?php echo $structThree['name'] ?>', layoutAdmin(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;">
                              ...
                              <div class="admin_layoutbox_widget_tabbed">
                                <ul class="sortablesForceInclude admin_content_sortable admin_layoutbox_widget_tabbed_contents">
                                  ...
                                  ?>
                                    <script type="text/javascript">
                                      window.addEvent('domready', function() {
                                        layoutAdmin("admin_content_<?php echo $structFour['identity'] ?>").store('contentParams', <?php echo Zend_Json::encode($structFourNE) ?>);
                                      });
                                    </script>
                                    <?php if( empty($structFourInfo) ): ?>
                                      <li style="display:none;" id="admin_content_<?php echo $structFour['identity'] ?>" class="disabled admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structFour['type'] . '_' . $structFour['name'] ?>">
                                    ...
                                    <?php else: ?>
                                      <li id="admin_content_<?php echo $structFour['identity'] ?>" class="admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structFour['type'] . '_' . $structFour['name'] ?>">
                                        <?php
                                          // showing admin title
                                          if( engine_in_array($structThree['name'], $allowedAdminTitle) && (!empty($structFourNE['params']['adminTitle'])) ):
                                            echo $this->translate($structFourNE['params']['adminTitle']);
                                          else:
                                            echo $this->translate($this->contentByName[$structFour['name']]['title']) ;
                                          endif;
                                        ?>
                                        <span class="open"> | <a href='javascript:void(0);' onclick="openWidgetParamEdit('<?php echo $structFour['name'] ?>', layoutAdmin(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;">edit</a></span>
                                        <span class="remove"><a href='javascript:void(0)' onclick="removeWidget(layoutAdmin(this));">x</a></span>
                                      </li>
                                    <?php endif; ?>
                                  <?php
        ...
        <div class='admin_layoutbox_footer<?php echo ( empty($this->pageObject->layout) || $this->pageObject->layout == 'default' ? '' : ' admin_layoutbox_footer_hidden' ) ?>'>
          <span>
            ...
            <span>
              <a href="javascript:void(0);" onclick="toggleGlobalLayout(layoutAdmin(this).getParent('div.admin_layoutbox_footer'));">
                <?php echo ( empty($this->pageObject->layout) || $this->pageObject->layout == 'default' ? "({$this->translate('hide on this page')})" : "({$this->translate('show on this page')})" ) ?>
              </a>
            </span>
          </span>
                    ...
                    <ul class="admin_content_sortable">
                      ...
                      ?>
                        ...
                        <?php if( empty($structThreeInfo) ): ?>
                          <li style="display:none;" id="admin_content_<?php echo $structThree['identity'] ?>" class="disabled admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structFour['type'] . '_' . $structFour['name'] ?>">
                            <?php echo $this->translate('Missing widget: %s', $structThree['name']) ?>
                            <span></span>
                          </li>
                        <?php else:?>
                        <li id="admin_content_<?php echo $structThree['identity'] ?>" class="admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?><?php if( !empty($structThreeInfo['special']) ) echo ' htmlblock' ?>">
                          ...
                          <span class="open"> | <a href='javascript:void(0);' onclick="openWidgetParamEdit('<?php echo $structThree['name'] ?>', layoutAdmin(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;">edit</a></span>
                          <span class="remove"><a href='javascript:void(0)' onclick="removeWidget(layoutAdmin(this));">x</a></span>
                        </li>
                        <?php endif;?>
                      <?php endforeach; ?>
                      ...
                      ?>
                        ...
                        <?php else:?>                        
                          <li id="admin_content_<?php echo $structThree['identity'] ?>" class="admin_content_cell admin_content_buildable admin_content_draggable admin_content_<?php echo $structThree['type'] . '_' . $structThree['name'] ?><?php if( !empty($structThreeInfo['special']) ) echo ' htmlblock' ?>">
                            ...
                            <span class="open"> | <a href='javascript:void(0);' onclick="openWidgetParamEdit('<?php echo $structThree['name'] ?>', layoutAdmin(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;"><?php echo $this->translate("edit") ?></a></span>
                            <span class="remove"><a href='javascript:void(0)' onclick="removeWidget(layoutAdmin(this));">x</a></span>
                          </li>
                        <?php endif;?>
                      <?php endforeach; ?>
    ...
    <div class='admin_layoutbox_pool'>
      ...
      <ul id='column_stock'>
        <?php foreach( $this->contentAreas as $category => $categoryAreas ): ?>
          <li>
            <div class="admin_layoutbox_pool_category_wrapper" onclick="layoutAdmin(this); layoutAdmin(this).getElement('.admin_layoutbox_pool_category_show').toggle(); layoutAdmin(this).getElement('.admin_layoutbox_pool_category_hide').toggle(); this.getParent('li').getElement('ul').style.display = ( this.getParent('li').getElement('ul').style.display == 'none' ? '' : 'none' );">
              ...
              <?php foreach( $categoryAreas as $info ):
                ...
                <?php //if( empty($info['canHaveChildren']) ): ?>
                  <li class="<?php echo $class ?>" title="<?php echo $this->escape($info['description']) ?>"<?php if( $onmousedown ): ?> onmousedown="<?php echo $onmousedown ?>"<?php endif; ?>>
                    ...
                    <span class="open"> | <a href='javascript:void(0);' onclick="openWidgetParamEdit('<?php echo $info['name'] ?>', layoutAdmin(this).getParent('li.admin_content_cell')); (new Event(event).stop()); return false;"><?php echo $this->translate("edit") ?></a></span>
                    <span class="remove"><a href='javascript:void(0);' onclick="removeWidget(layoutAdmin(this));">x</a></span>
                  </li>
                <?php /* //else: ?>
...
<script type="application/javascript">
  ...
  scriptJquery('.core_admin_main_layout_content').addClass('active');
</script>
