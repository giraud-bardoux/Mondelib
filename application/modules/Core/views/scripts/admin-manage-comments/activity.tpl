 ...
 */
?>
<?php echo $this->partial('_admin_breadcrumb.tpl', 'core', array('parentMenu' => "core_admin_main_manage", 'parentMenuItemName' => 'core_admin_main_manage_comments', 'lastMenuItemName' => 'Comments on Activity Feeds')); ?>
...
<div class="admin_common_top_section">
  <h3><?php echo $this->translate("Manage Comments on Activity Feeds") ?></h3>
  <p><?php echo $this->translate("This page lists all of the comments your users have posted on activity feeds on your site. You can use this page to monitor these comments and delete offensive material if necessary. Entering criteria into the filter fields will help you find specific comments. Leaving the filter fields blank will show all of the comments on activity feeds on your social network.") ?> </p>
</div>  
...
<?php if($count > 0) { ?>
  ...
  <div class="admin_table_form">
    <form id='multimodify_form' method="post" action="<?php echo $this->url(array('action'=>'multi-activitycommentdelete'));?>" onSubmit="multiModify()">
            ...
            <?php foreach( $this->paginator as $item ): ?>
              <tr>
                ...
                <td data-label="<?php echo $this->translate("Comment") ?>">
                  <?php echo $this->partial('_activitycommentcontent.tpl', 'comment', array('comment' => $item)); ?><?php //echo $this->string()->truncate(Engine_Text_Emoji::decode($item->body), 45, '...') ?></td>
                ...
                <td class='admin_table_options _comment_options'>
                  <a class="smoothbox" href='<?php echo $this->url(array('action' => 'read-comment', 'id' => $item->getIdentity(), 'resource_type' => 'activity_comment')) ?>'>
                    <?php echo $this->translate("Read Comment") ?>
                  </a>
                  |
                  <?php $action = Engine_Api::_()->getItem("activity_action", $item->resource_id); ?>
                  <?php if($action) { ?>
                  <a target='_blank' href='<?php echo $action->getHref();?>'>
                    <?php echo $this->translate("View Feed") ?>
                  </a>
                  |
                  <?php } ?>
                  <a class='smoothbox' href='<?php echo $this->url(array('action' => 'delete', 'id' => $item->getIdentity(), 'resource_type' => 'activity_comment'));?>'>
