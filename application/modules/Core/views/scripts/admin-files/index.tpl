 ...
 */
?>
<?php echo $this->partial('_admin_breadcrumb.tpl', 'core', array('parentMenu' => "core_admin_main_layout", 'parentMenuItemName' => 'core_admin_main_layout_files', 'childMenuItemName' => 'core_admin_main_layout_files_manager')); ?>
...
<?php if( engine_count($this->paginator) ): ?>
  ...
  <div class="select_fmm_all"><input onclick='selectAll(this);' type='checkbox' class='checkbox' /><?php echo $this->translate("Select All"); ?></div>
  <ul class="fmm_media_list">
      <?php foreach ($this->paginator as $item): ?>
        <?php $storage = Engine_Api::_()->getItem('storage_file', $item->storage_file_id);
        if($storage) {
        $path = $storage->map();
        $copyPath = ($storage->service_id == 2) ?  $path : $path; 
        ?>
