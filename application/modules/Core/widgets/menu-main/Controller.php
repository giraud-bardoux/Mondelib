...
class Core_Widget_MenuMainController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    if(Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('serenity')) {
      ...
      if(!empty($viewer_id))
        empty($headerloggedinoptions) ? $this->setNoRender() : ((!engine_in_array('mainMenu', $headerloggedinoptions)) ? $this->setNoRender() : '');
      else 
        empty($headernonloggedinoptions) ? $this->setNoRender() : ((!engine_in_array('mainMenu', $headernonloggedinoptions)) ? $this->setNoRender() : '');
    }
    ...
    $requireCheck = Engine_Api::_()->getApi('settings', 'core')->getSetting('core.general.portal', 1);
