...
class Core_Model_DbTable_Tags extends Engine_Db_Table
{
  ...
  public function setTagMaps(Core_Model_Item_Abstract $resource, $tagger, array $tags)
  {
    ...
    foreach ($tagObjects as $tag) {
      if (
        !$existingTagMaps->getRowMatching(
          array(
            'tag_type' => $tag->getType(),
            'tag_id' => $tag->getIdentity(),
          )
  ...
  public function getTopHashtags($limit)
  {
    ...
    foreach ($hashtagMaps as $hashtag) {
      $hashtagNames[] = array('tagName' => '#' . $hashtag->text, 'tagCount' => $hashtag->tag_count);
    }
    return $hashtagNames;
  }
  // Utility
  ...
  public function tagCloudItem($params = array()) {
    ...
    if(isset( $params['resource_id']) && !empty($params['resource_id'])) {
      $select->where($tableTagName.'.resource_id =?', $params['resource_id']);
    }

    $select->columns(array('itemCount' => ("COUNT($tableTagName.tagmap_id)")));

    if (empty($params['fetchtype']))
