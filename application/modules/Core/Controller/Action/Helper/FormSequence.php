 ...
 * @version    $Id: FormSequence.php 9747 2012-07-26 02:08:08Z john $
 * @author     John
 */
  ...
  extends Zend_Controller_Action_Helper_Abstract
{
  ...
  public function doSubmit()
  {
    if( $this->getActionController()->getRequest()->isPost() )
    {
      foreach( $this->getPlugins() as $plugin )
      {
        if( $plugin->isActive() && $this->checkSubscriptionStep($plugin) )
        {
  ...
  private function checkSubscriptionStep($plugin)
  {
    if( !($plugin instanceof Payment_Plugin_Signup_Subscription) ) {
      return true;
    }

    $packages = Engine_Api::_()->getDbtable('packages', 'payment')->getEnabledPackages(true);
    if (empty($packages->toArray())) {
      return false;
    }
    return true;
  }
