...
class Core_Api_Mail extends Core_Api_Abstract
{
    ...
    {
      if( null === $this->_log ) {
        if (@is_file(APPLICATION_PATH . '/temporary/log/mail.log')) {
            @chmod(APPLICATION_PATH . '/temporary/log/mail.log', 0777);
            $log = new Zend_Log();
    ...
    public function send(Zend_Mail $mail)
    {
        if( $this->_enabled ) {
            ...
            if( $this->_queueing ) {
                ...
                // Single
                if( engine_count($mail->getRecipients()) <= 1 ) {
                    $mailTable = Engine_Api::_()->getDbtable('mail', 'core');
                    $mailTable->insert(array(
                        'type' => 'zend',
                        'body' => serialize($mail),
                        'recipient_count' => engine_count($mail->getRecipients()),
                    ));
                }
                ...
                // Multi
                    ...
                    $mail_id = $mailTable->insert(array(
                        ...
                        'recipient_count' => engine_count($recipients),
                        'recipient_total' => engine_count($recipients),
                ...
                foreach( $mail->getRecipients() as $oneRecipient ) {
                    ...
                    if (30 <= engine_count($mailClone->getRecipients())) {
                        $this->sendRaw($mailClone);
                        $mailClone->clearRecipients();
                        $mailClone->addTo( $mailClone->getFrom() );
                    }
                }
                if( 1 <= engine_count($mailClone->getRecipients()) ) {
                    $this->sendRaw($mailClone);
        ...
        // Queue
            ...
            foreach( $recipients as $oneRecipient ) {
                if( $oneRecipient instanceof Core_Model_Item_Abstract ) {
									...
									if(!engine_in_array($type, array('user_otp', 'core_welcome', 'core_verification', 'notify_admin_user_signup')) && empty($oneRecipient->verified)) return;
    ...
    public function sendSystemRaw($recipient, $type, array $params = array())
    {
        ...
        foreach( $recipients as $recipient ) {
            ...
            if( is_string($recipient) ) {
                ...
                if( null !== $user ) {
                    $enabledEmailTypes = Engine_Api::_()->getDbtable('emailSettings', 'user')->checkEnabledEmail($user, $type);
                    if(empty($enabledEmailTypes)) continue;
                    $recipient = $user;
                }
            } else if(!empty($recipient->disable_email) && $recipient->user_id && $type != "core_lostpassword") {
                continue;
            }
            ...
            if( $recipient instanceof Core_Model_Item_Abstract ) {
                ...
                //If member is not verifed then do not send email
                if(!engine_in_array($type, array('user_otp', 'core_welcome', 'core_verification', 'notify_admin_user_signup','core_lostpassword')) && empty($recipient->verified)) return;
                ...
                if( !Zend_Locale::isLocale($recipientLanguage) ||
                    ...
                    !engine_in_array($recipientLanguage, $translate->getList()) ) {
                    $recipientLanguage = $translate->getLocale();
                }
            ...
            if($recipient instanceof Core_Model_Item_Abstract ) {
                if(!is_null($mailTemplate->member_level) && !engine_in_array($recipient->level_id,explode(",", $mailTemplate->member_level))){
                    return false;
                }
            }
