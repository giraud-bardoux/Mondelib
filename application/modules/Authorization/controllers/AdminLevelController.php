...
class Authorization_AdminLevelController extends Core_Controller_Action_Admin
{
  public function indexAction()
  {
    ...
    if( engine_count($defaultLevelDuplicates) != 1 ) {
      ...
      foreach( array_keys($defaultLevelDuplicates) as $key ) {
        ...
        if( engine_count($defaultLevelDuplicates) <= 0 ) {
  ...
  public function deleteselectedAction()
  {
    ...
    $this->view->count = engine_count(explode(",", $ids));
...
//     }
  ...
  public function mapProfileTypeAction()
  {
    ...
    if ((engine_count($mappedProfileTypeIds) >= engine_count($profileTypes)) && !$mappedProfile) {
