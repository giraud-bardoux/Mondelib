  ...
  implements Authorization_Model_AdapterInterface
{
  ...
  public function getAllowed($resource, $role, $action)
  {
    ...
    // No data
    if( null === $data || (is_array($data) && engine_count($data) === 0) ) {
  ...
  public function setAllowed($resource, $role, $action, $value = null, $nonBooleanSettings = array())
  {
    $privacySettings = array('auth_view', 'auth_comment', 'auth_tag', 'auth_photo', 'auth_event');
    // Can set multiple actions
    if( is_array($action) )
    {
      foreach( $action as $key => $value )
      {
        $this->setAllowed($resource, $role, $key, $value, $nonBooleanSettings);
      }
    ...
    if( null !== Authorization_Api_Core::getConstantKey($value) && !engine_in_array($action, $nonBooleanSettings) )
    {
      $row->value = $value;
      $row->params = null;
    }
    ...
    {
      if( (empty($value)
        || (is_array($value) && ($values = array_filter($value)) && empty($values)))
        && engine_in_array($action, $privacySettings) ) {

        // don't change auth_xxx privacy
        return $this;
      }
