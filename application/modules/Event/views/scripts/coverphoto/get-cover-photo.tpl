...
<?php $levelId = $this->user->level_id;
  ...
  $coverphoto = Engine_Api::_()->authorization()->getPermission($levelId, 'event', 'coverphoto');
...
<?php if (empty($this->uploadDefaultCover)) : ?>
  ...
  <?php elseif (!empty($coverId = Engine_Api::_()->getApi("settings", "core")->getSetting("eventcoverphoto.preview.level.id.$levelId"))) : ?>
    <div class="profile_cover_photo cover_photo_wap b_dark">
  ...
  <?php if (!empty($this->can_edit)) : ?>
    <div id="cover_photo_options" class="profile_cover_options">
      <ul class="edit-button">
        <li>
          <?php if (!empty($this->event->coverphoto) && $this->photo) : ?>
          ...
          <ul class="profile_options_pulldown">
            <li>
              <a href='<?php echo $this->url(array('action' => 'upload-cover-photo', 'event_id' => $eventId), 'event_coverphoto', true); ?>'
            ...
            <li>
              <?php echo $this->htmlLink(
                array('route' => 'event_coverphoto', 'action' => 'choose-from-albums', 'event_id' => $eventId, 'recent' => 1),
                $this->translate('Choose from Albums'),
                array(' class' => 'profile_cover_icon_photo_view smoothbox')
              ); ?>
            </li>
            <?php if (!empty($this->event->coverphoto) && $this->photo) : ?>
              <li><a  href="javascript:document.coverPhoto.reposition.start()" class="cover_reposition profile_cover_icon_move ajaxPrevent">
                      <?php echo $this->translate("Reposition"); ?></a>
              </li>
              <li>
                <?php echo $this->htmlLink(
                  array('route' => 'event_coverphoto', 'action' => 'remove-cover-photo', 'event_id' => $eventId),
      ...
      <?php if (!empty($this->event->coverphoto)) : ?>
...
<?php else : ?>
  ...
  <div class="profile_cover_photo cover_photo_wap b_dark">
    <?php if(!empty($coverId = Engine_Api::_()->getApi("settings", "core")->getSetting("eventcoverphoto.preview.level.id.$levelId"))) : ?>
      <img src="<?php echo Engine_Api::_()->storage()->get($coverId, 'thumb.cover')->map()?>"
        align="left" class="cover_photo" style="top:<?php echo $this->topPosition?>px" />
        <div class="cover_tip_wrap is_hidden">
          <div class="cover_tip">
  ...
  <div id="cover_photo_options" class="profile_cover_options">
    <ul class="edit-button">
      <li>
        <?php if (Engine_Api::_()->getApi("settings", "core")->getSetting("eventcoverphoto.preview.level.id.$levelId")) : ?>
          <span class="profile_cover_btn">
            <i class="fa fa-camera" aria-hidden="true"></i>
          </span>
        <?php else : ?>
          ...
          <ul class="profile_options_pulldown">
            <li>
              <a href='<?php echo $this->url(array(
                ...
                'level_id' => $this->level_id), 'event_coverphoto', true); ?>' class="profile_cover_icon_photo_upload smoothbox">
