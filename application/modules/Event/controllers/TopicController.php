 ...
 * @version    $Id: TopicController.php 9747 2012-07-26 02:08:08Z john $
 * @author     John
 */
...
class Event_TopicController extends Core_Controller_Action_Standard
{
  ...
  public function viewAction()
  {
    ...
    $this->view->canEdit = $topic->canEdit($viewer);
    $this->view->canDelete = $topic->canDelete($viewer);
    $this->view->canPostCreate = $canPostCreate = $topic->canPostCreate($viewer);
    $this->view->canPostEdit = $topic->canPostEdit($viewer);
    $this->view->canPostDelete = $topic->canPostDelete($viewer);
    $this->view->canAdminEdit = Engine_Api::_()->authorization()->isAllowed($event, null, 'edit');
    $this->view->canPost = $canPost = $event->authorization()->isAllowed(null,  'topic_create');
    ...
    if( !$event || !$event->getIdentity() || ((!$event->approved) && !$event->isOwner($viewer)) ) {
  ...
  public function createAction()
  {
    ...
    echo json_encode(array('status' => true, 'redirectURL' => $post->getHref()));die;
  }
  ...
  public function postAction()
  {
    ...
    try
    {
      ...
      // Activity
      $action = $activityApi->addActivity($viewer, $event, 'event_topic_reply', null, array('child_id' => $topic->getIdentity()));
      if( $action ) {
  ...
  public function stickyAction()
  {
    ...
    //echo json_encode(array('status' => true, 'redirectURL' => $topic->getHref()));die;
