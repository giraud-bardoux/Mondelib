...
class Sesiosapp_Model_DbTable_NotificationTypes extends Engine_Db_Table
{
  /**
  ...
  public function getDefaultNotifications()
  {
    ...
    $select = $this->select()
      ...
      ->where('sesios_enable_pushnotification = ?', 1);
  ...
  public function setDefaultNotifications($values)
  {
    ...
    if( !empty($defaults) ){
      ...
      $this->update(
                    array('sesios_enable_pushnotification' => '1',), array('`type` IN(?)' => $defaults));
   
      $this->update(
        array('sesios_enable_pushnotification' => '0',), 
        array('`type` NOT IN(?)' => $defaults));
        
    } else {
      $this->update(array('sesios_enable_pushnotification' => '0'), array('`sesios_enable_pushnotification`' => '1'));
    }
